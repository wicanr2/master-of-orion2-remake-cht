"""唯讀匯出原版同化鏈及 Charismatic／Repulsive 玩家欄位消費端。"""
...
OUT = os.environ.get("MOO2_RE_OUT", "/out/assimilation-traits.json")
...
def raw(ea, size):
    return {"ea": f"0x{ea:X}", "size": size,
            "bytes": (ida_bytes.get_bytes(ea, size) or b"").hex()}


def operand_hits():
    needles = ("+8B2h", "+8B3h", "+12Eh", "+12Fh", "+0B4h", "+0C8h")
    out = []
...
def main():
    ...
    report = {
        ...
        "tool": {"name": "IDA Pro", "version": ida_kernwin.get_kernel_version(),
                 "script": "tools/ida/audit_assimilation_traits.py"},
        ...
        "tables": {"raw_government_assimilation_rate": raw(0xDD4F5, 8)},
