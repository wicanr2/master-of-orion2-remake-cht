"""非破壞性匯出 MOO2 1.31 難度全域的全部直接讀寫端與 owner 函式。"""
...
def direct_sites(symbols):
    rows = []
    seen = set()
    for ref in idautils.XrefsTo(DIFFICULTY, 0):
        ...
        rows.append({
            ...
            "window": [instruction(ea) for ea in items[max(0, index - 20):min(len(items), index + 21)]],
        })
    return rows
...
def callers(start_ea, symbols):
    rows = []
    for ref in idautils.XrefsTo(start_ea, 0):
        owner = ida_funcs.get_func(ref.frm)
        if owner is None:
            continue
        items = list(idautils.FuncItems(owner.start_ea))
...
def main():
    ...
    sites = direct_sites(symbols)
    owners = sorted({int(site["owner_start"], 16) for site in sites})
    report = {
        "schema": "moo2.ida.difficulty-consumers.v1",
        "evidence_scope": "static_only",
        "mutation": "none",
        "semantic_status": "reviewed_against_raw_instructions",
        "tool": {"name": "IDA Pro", "version": ida_kernwin.get_kernel_version(),
                 "script": "tools/ida/audit_difficulty_consumers.py"},
        ...
        "difficulty_global": {
            "ea": f"0x{DIFFICULTY:X}",
            "original_name": ida_name.get_name(DIFFICULTY) or "<unnamed>",
            "raw_byte": (ida_bytes.get_bytes(DIFFICULTY, 1) or b"").hex(),
            "direct_sites": sites,
        },
        "owner_functions": {
            f"0x{start:X}": {"function": function_record(start, symbols),
...
try:
    ...
    output = os.environ.get("MOO2_RE_OUT", "/tmp/difficulty-consumers.json")
