"""唯讀匯出事件 2 彗星的建立、逐回合檢查、攔截與殖民地回寫資料流。"""
...
OUT = os.environ.get("MOO2_RE_OUT", "/out/event-comet.json")
...
ROOTS = {
    ...
    "raw_Comet_Interception_Strength": 0x23B28,
    "raw_Comet_Impact_Colony": 0x23780,
    "raw_Event_Colony_Conflict_Query": 0x23E60,
}
...
def named_comet_functions():
    matches = []
    for ea in idautils.Functions():
        name = idc.get_name(ea) or ""
        if "comet" in name.lower():
            matches.append(function(ea))
    return matches
...
def main():
    ...
    report = {
        ...
        "tool": {"name": "IDA Pro", "version": ida_kernwin.get_kernel_version(),
                 "script": "tools/ida/audit_event_comet.py"},
        ...
        "named_comet_functions": named_comet_functions(),
    }
