...
6. `sub_D73D4 @ 0xD73D4` 每回合反向掃描 67 位領袖：type 0 交給 `sub_D6FDA`，type 1 依 status 0／1 分別交給 `sub_D7171`／`sub_D7078`。這證明 AI 不只雇人，也持續重做艦艇與殖民地任命。
7. `sub_D6FDA @ 0xD6FDA` 只考慮同 owner、狀態／任命欄位合格且尚無領袖的船，並以多個 raw ship 欄位 `+0x10/+0x12/+0x15/+0x16/+0x72` 作支配比較，最後呼叫 `sub_98C23` 寫入任命。
...
- `sub_D7171/sub_D7078` 的殖民地 raw 評分涉及 remake 尚無一對一欄位；規格採人口與產出排序的玩家可見近似。招募、付費、技能 gate、單一 offer、延遲一回合與 30 回合拒絕 cooldown 則按已證實資料流實作。
