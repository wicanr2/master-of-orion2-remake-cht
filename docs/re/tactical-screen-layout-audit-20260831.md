...
差異由後續裁切收斂為 Amoeba 主畫面生成座標、畫面外進場移動、Star Base 外環級別、右下縮圖
與自由座標移動規則；沒有原版證據前不得以把怪物任意移出畫面來偽造首幀相似度。

## 戰術縮圖與視窗框（2026-08-31）

原版控制甲板右下裁切放大四倍後，可量測行星中心約 `(519,418)`、Star Base 約 `(532,417)`、
兩艘 Frigate 約 `(538,413)/(538,415)`，以及橘色 Amoeba 約 `(571,416)`。前三類已知戰場座標
共同支持縮圖投影 `miniX=507+screenX*3/40`、`miniY=404+screenY/16`；四角框寬約 48、高約 22，
亦分別對應 640×351 的目前可視區。證據等級為**強推論**：它來自同狀態像素量測，尚未由
IDA 找到原版定點運算 consumer。

橘點反投影約為戰場 `(853,192)`，位於 640px 畫面右側，足以解釋原版首幀只在縮圖看見
Amoeba。remake 只在縮圖使用這個單幀反推點，不把它寫回玩法座標，也不讓不可見敵人永久卡在
畫面外。右下縮圖已由 typed 行星／艦艇狀態重建；Star Base 選艦環則改用與裁切吻合的
`COMBAT#33`。Amoeba 精確生成座標與原版縮圖內部公式仍保留未知。
...
本體尺度接近、外環選錯，故不再作為獨立缺口。仍未關閉的是 Amoeba 主畫面生成座標、畫面外
進場移動、縮圖定點公式的 IDA 證據，以及自由座標玩法模型。
