...
## 這 75 項煉出來的規則(先讀這一段,底下是參考資料)

> 編號項裡的教訓高度重複——「擋門理由會過期」出現十次、「兩條戰鬥路徑」
> 出現在第 69–72 項每一項。合併成下面 12 條;**要用的是這些,不是逐項讀完**。
> 每條後面是產生它的項次,要看證據再去翻。
...
1. **擋門理由會過期,而且可能當初就寫錯。** 「為什麼這項還沒做」那句話在寫下的那天
   ...
   〔59 60 64 65 66 67 69 71 72 73〕
2. **「元件表裡有」不等於「效果有接」。** 一項可以在表裡、花得了錢,而程式碼沒有任何
   地方讀它。盤點時要分開問這兩件事;偵測法是掃名字在表以外出現幾次,零次就是裝飾品。〔73〕
3. **任何清單都會過期,包括警告別人清單會過期的那份清單。** 做完一項就回去刪那一行。〔73〕
...
7. **一個元件常有兩個效果,只接一半不會有任何徵兆。** 從單一檔案回推元件效果會漏東西;
   要從**手冊那一段的每一句**回推。〔69 72〕
8. **資料層跟著實作層缺,是很難發現的洞。** 單看資料層會以為「這一組本來就只有這些」。〔71〕
9. **測試名稱裡的「只」是一種斷言**,而它斷的往往是你當時的理解,不是手冊。〔72〕

### 三、關於逆向

10. **一手來源永遠贏二手推論。** 一手 = 執行檔立即數、LBX 位元組、手冊行文;
    二手 = 檔名、美術字樣、命名習慣。**openorion2 沒有 ≠ 沒有真值**——反組譯全都有。〔15 66 68〕
11. **不要 grep `.asm`,要查 IDA 資料庫的 xref 圖。** `.asm` 是攤平的文字,沒有交叉參考。
    而且除錯符號表的 `obj1+偏移` **不是**線性位址(差一個 object base)。〔74〕
...
- **狀態指紋變了就代表持久化狀態變了**,必須查出原因,不可因為「改動有理」就放過。〔72〕
...
| 12 | 多人連線 11 個畫面 | — | ✅ `MP_Setup`(`cmd/moo2/multiplayer.go`)與 `Hotseat`(`cmd/moo2/hotseat.go`)2026-08-07 已建,版面座標取自反組譯(見下方第 4 項)。`Net_Next_Turn`(第 30 項)與 `Choose_Net_Plyrs`(第 30 項)2026-08-07 已建。`Modem_Setup`/`NullModem_Setup`/`Comm Info` **不做**(硬體已不存在)。`Join_Net`/`Generic_Net_Info`/`SendGet_Net_Info` 是同一張畫面的不同狀態(第 30 項),`Choose_Multi_Net_Game` 見第 30 項。**11 張全部結案:8 做 / 3 不做** |
...
**教訓與第 12 項同一條**:斷言一旦寫進文件就會被當成現況引用,而程式碼會往前走、文件不會。
...
>    第 75 項訂下的分界——**其餘所有文件只留現況,錯誤推導的全文只留在這裡**。
>
...
| 原本的斷言 | 現況 |
|---|---|
| **同星系多殖民地** | ✅ 第 25 項:拓殖/前哨站的對象改成**行星**;人造行星改造完的天體也真的能殖民了 |
| **戰術格子的獨立戰機單位** | ✅ 第 28 項:中隊在格子上有自己的位置(出擊→飛向目標→貼身開火→返航補給→再出擊)。⚠ 同時訂正「中隊數 4/2」——那是把 p.127 的 **Shots** 欄讀成了中隊人數,正確答案是**一律 4 架** |
| AI 的遷移設定 | ✅ **不是缺口**(第 29 項):那個欄位的五個寫入者沒有一個在 AI 的程式碼裡——原版的 AI 也不設集結點 |
| AI 的同星系多殖民地 | ✅ 第 25 項:`aiExpand` 的候選集加進「自己已有殖民地的星系」;順帶修好入侵提早翻面的 bug |
| 遷移連線的顯示開關沒有 UI | ✅ 第 24 項(⚠ 那一列不是原版版面,原版有一整個設定畫面) |
| `Clear_All` 集結點沒有 UI 入口 | ✅ 第 24 項。⚠ 同時訂正——**ALL 鈕不是 Set_All**,手冊兩處明說它是「全選/全不選艦艇」 |
| 遷移確認框 | ✅ 第 27 項(版面取自 `Confirmation_Box_`)。⚠ 同時訂正——原版那個條件是**怪獸**不是艦隊 |
| `Command_Points` 專屬畫面沒做 | ✅ `cmd/moo2/commandpoints.go`(第 14 項) |
| 殖民地地表缺「擺放的最後一段抖動」 | ✅ 第 15 項 |
| 殖民地地表缺植被層 | ✅ 第 15 項(COLVEGGI,13 組 × 8 張 = 104 對上資產數) |
| 艦隊圖示 8 幀「動畫沒做」 | ✅ 第 23 項:**原版就不會動**(每次繪製前歸零幀號),假缺口 |
| 一般星球閃爍「三個常數沒追出來」 | ✅ 第 23 項:**出貨版是死碼**(沒有任何地方啟動它),假缺口 |
| 鍵盤快捷鍵 | ✅ 第 23 項全數接完(F1/F2、F5/F6、F9、F10、ALT+F9)。ALT+F1..F8 的設定開關仍不碰(PDF 邊欄標籤有 off-by-one 風險) |
| 星圖遷移連線 | ✅ 第 24 項。**星圖 4 層全部到齊**:蟲洞(35)、遷移連線(57)、星星、星門(47)+ 星雲(44)+ 外交燈號(50) |
| 多艦隊模型 | ✅ 第 24 項(資料層)+ 第 24 項(艦隊列表)+ 第 24 項(分艦隊) |
| RELOCATE 鈕的原版語意 | ✅ 第 24 項:兩段點選 + 四條合法性規則,已接 |
| 分艦隊 UI | ✅ 第 24 項(⚠ 入口是 remake 自己加的,原版在右側艦艇格) |
...
2. ~~星系/行星生成表~~ → **已接進 remake**(`gamedata/galaxygen.go`,commit `f8bbcbd`)。
...
3. **一星多行星** → 原版每顆恆星 1–5 顆行星各佔一條軌道;remake 的 `Stars`/`Planets`
...
4. ~~獨立 Colony 畫面 + Event 畫面~~ → **兩個都已完成**(2026-08-06)。
...
5. **地面戰解算**(`Resolve_Ground_Combat_` / `Ground_Combat_Round_`)→ 取代目前沿用一代 1oom 的借用結構。

### 第三梯:補完整性

6. ~~NEW GAME 設定畫面用估計座標、只有 3 個設定能選~~ → **2026-08-07 全部重挖**
...
## 二、逐畫面重建與子系統補齊(2026-08-07)
    ...
    | 畫面 | 狀態 |
    |---|---|
    | 地面戰 | ✅ 2026-08-07(第一個實例,見第 4 項)|
    | 軌道轟炸 | ✅ 2026-08-07(第 4 項)|
    | NEW GAME 設定 | ✅ 2026-08-07(第 6 項,順便修掉 PLAYERS 被當 RACE 用)|
    | 殖民地 | ✅ 2026-08-07(第 7 項,框架 = COLPUPS.LBX#5)|
    | 種族選擇 | ✅ 2026-08-07(第 6 項,順便修掉左右擺反)|
    | 艦艇設計 | ✅ 2026-08-07(第 6 項,六格不等距;右上兩個面板待追)|
    | 多人設定 / 熱座交接 | ✅ 2026-08-07(第 4 項)|
...
8. **TECH LEVEL 接上第一個 gameplay 效果:曲速前的 FTL 限制**(2026-08-07)。

    第 6 項留下的「TECH LEVEL 只存設定、不影響 gameplay」不再完全成立。手冊直引
...
9. **行星表面 + 建築 sprite 擺放:先查清楚為什麼不能照上面那套方法做**(2026-08-07 調查)。
    ...
    ### **行星表面格點:表抽出來了,第 9 項的「獨立工程」判斷只對了一半**(2026-08-07 續)。


        第 9 項的結論是「格點→螢幕座標是烘在資料段的幾何表,沒有公式可抄」——**這句是對的**。
    ...
    ### **原版建築表挖出來:所有「估計的」建造成本換成真值**(2026-08-07)。
        ...
        第 9 項在追建築 sprite 時,`Real_Building_Name_` @ 0xBB40D 只有兩行——
    ...
    ### **半透明標記索引:index >= 0xF0 從來不是顏色**(2026-08-07)。
        ...
        第 9 項留下的「陰影是洋紅」問題,追下去發現不只是陰影,而是 remake 一直誤解了
    ...
    ### **中段還給行星表面,建造佇列搬回原版的彈出視窗**(2026-08-07,第 9/9/9/9 項的收尾)。


        第 9 項推翻的那句「中段(y 159..423)原版畫行星表面」其實還有更深一層:remake 把
    ...
    ### **原版 PRNG 照抄 + 建築擺放換成原版演算法 + 地表底圖接上**(2026-08-07,第 9 項的續)。
        ...
        - ~~**道路** `Draw_Road_List` 沒畫。~~ 第 15 項已補。
        - 衛星的**選取態閃爍**(`sub_BE271` 那條分支)沒做——remake 沒有點選衛星的互動。

10. **星圖:艦隊圖示換成原版的、旗色順序修正、拿掉浮在星圖上的研究文字**(2026-08-07)。

    ### 先把星圖的圖層順序抄下來(`Draw_Main_Main_Screen_` @ 0x8440E)

    | # | 圖層 | 位址 | remake |
    |---|---|---|---|
    | 1 | `Draw_Wormhole_Links_` | 0x85593 | ✅ 資料模型 + 連線 + 航行機制(第 11 項)|
    | 2 | `Draw_Relocation_Links_` | 0x85320 | ❌ |
    | 3 | `Draw_Stars_` → 逐星 `Draw_A_Star_` | 0x85550 / 0x83B02 | ✅ sprite 已接(第 10 項);閃爍動畫未做 |
    | 4 | `Draw_A_Gate_Icon_`(迴圈) | 0x83741 | ❌ |
    | 5 | `Print_Star_Names_` | 0x88CB7 | ⚠ 位置已對齊(第 10 項);字型樣式/描邊未做 |
    ...
    ### **星圖:星球換成原版 sprite**(2026-08-07,第 10 項的續)。
        ...
        ### 星名位置(`Print_A_Star_Name_` @ 0x87768,併入第 10 項)
        ```
...
11. **蟲洞:把星圖第 1 層卡住的資料模型補上**(2026-08-07)。

    第 10/10 項留下的結論是「剩下的層卡的是資料模型不是繪圖」。這一項就是去補模型——
...
12. **原版 48 棟建築,remake 到底缺哪幾棟?——把 8 個沒建模的編號查清楚**(2026-08-07)。
    ...
    | 42 | Stellar Converter(行星版) | 1000 | 6 | 0 | ✅ 2026-08-07 補上,見第 12 項 |
    ...
    ### **恆星轉換器(行星版)補上 + 一個乾淨的負面結果**(2026-08-07,第 12 項的續)。


        第 12 項列出三棟真的缺的建築,並寫下「下一輪的路線是從 `colony + 0x136 + id` 的旗標
        ...
        - **Galactic Currency Exchange**(18):效果四條路都查不到(第 12 項),而這次的位移掃描
    ...
    ### **飛彈基地/地面砲台其實早就算得出來——是接線漏了,不是模型缺了**(2026-08-07,第 12 項的訂正)。


        飛彈基地與地面砲台的空間預算模型**早就在**,第 12 項把它當成缺口是漏查:
...
13. **盤點:「最大的系統級缺口」那四條全部已經做掉了**(2026-08-07)。

    第 12 項的教訓是「**別編一個做不到的理由,先 grep**」。這一項把同一把尺對準文件自己:
...
14. **指揮點數視窗建起來 + 一個「畫面自己打自己臉」的快取陳舊值**(2026-08-07)。

    第 13 項核實後的清單裡,`Command_Points` 專屬畫面是最小的一項。做掉它。
...
15. **殖民地地表的道路畫出來了,順手在原版資料裡撞到兩個位元組級的錯**(2026-08-07)。

    第 9 項留下的「道路沒畫」補完。`cmd/moo2/colonyroads.go`,護欄 `colonyroads_test.go`。
    ...
    ### **房屋抖動補完 + 母星的國會大廈一直沒畫**(2026-08-07,接第 15 項)。


        第 15 項說「道路位置對不上原版,因為抖動沒補」。這一項把抖動補上,順帶發現一件更基本的事:
    ...
    ### **殖民地地表的植被層(第 15 項發現的缺口,同日補上)**(2026-08-07)。
        ...
        **remake 的 `colonyCellCenter` 已經是同一個算式**,直接用。
        ...
        密度用的 `sub_B67E4` 就是第 15 項提到「Δa/Δb 對調了兩筆」的那張表。差別是:
...
16. **星雲:星圖 4 層裡的第 1 層,而且是有規則的地形不是裝飾**(2026-08-07)。

    `internal/shell/nebula.go`(規則)+ `cmd/moo2/nebula.go`(圖與判定),
    護欄 `nebula_test.go`。第 10 項列的星圖 4 層,做掉第 1 層。
...
17. **星圖移動換成秒差距模型:四條手冊規則從「無處可掛」變成可實作**(2026-08-07)。

    `internal/gamedata/starlane.go`(數值)+ `internal/shell/starlane.go`(接線),
    護欄 `starlane_test.go`。第 16 項留下的移動懲罰,前置補完。
    ...
    **星數 → 檔位。** `Galaxy_Size_From_N_Stars_` @ 0x798D2 的門檻是 **20 / 36 / 54 / 72**。
    ...
    ### **星圖航線模型:三條規則的共同前置補完**(2026-08-07)。
        ...
        `internal/shell/route.go`,護欄 `route_test.go`。第 17 項留下的三項全部接上。
...
18. **拓殖基地:國會大廈那個坑的另一半**(2026-08-07,task #46 的可實作部分)。

    第 15 項補上母星的國會大廈(編號 9)時,只治了一半。**編號 11 Colony Base 是完全對稱的
...
19. **銀河貨幣交易所:它根本不是建築**(2026-08-07)。

    第 12 項留下的兩個「完全沒有」的編號,解掉一個。
...
20. **AI 主動請求會談 + 星圖上緣的請求燈**(2026-08-07)。
...
21. **訂正:「手冊全文搜尋零命中」是假陰性——PDF 的連字騙了我**(2026-08-07)。
...
22. **查證:一次性改造完成後不佔地表格子**(2026-08-07,task #46 的最後一個未查證項)。

    第 18 項留下的問題:「一次性改造(17 Gaia / 37 Soil Enrichment / 44 Terraforming)
...
23. **黑洞的旋渦動畫:規則完整就做,不完整就不做**(2026-08-07)。

    星圖上的黑洞從第 10 項起就用對了圖(BUFFER0#187),但它是**靜止**的。
    原版不是——黑洞會轉。
    ...
    **下次再遇到就該去修 `decodeAsset` 本身**,不要再加第五層。
    ...
    ### **訂正兩條:艦隊圖示不會動、一般星球也不會閃——都是原版行為,不是缺口**(2026-08-07,接第 23 項)。
        ...
        | ALT + F9 | 從星圖載入遊戲 |
        F9 特別值得做:remake 第 17 項已經把秒差距模型建好了(`ParsecsBetweenStars`),


    ### **手冊的星圖快捷鍵接上:F1/F2、F5/F6、F9**(2026-08-07,第 23 項副產品的實作)。


        第 23 項追 `Cycle_Ship_Icons_` 時把手冊的快捷鍵段落掃出來了。這一項把**行文中直接寫死**
        的那幾個接上(邊欄標籤的 ALT+Fn 那組仍然不碰,理由見第 23 項)。
...
24. **多艦隊模型:把「全帝國只有一支艦隊」這個限制拆掉**(2026-08-07,第一階段)。
    ...
    ### **遷移連線:星圖 4 層裡的最後一層補上**(2026-08-07,第 24 項的續)。


        原版 `Draw_Relocation_Links_` @ 0x85320 是主畫面圖層順序的第 2 層。它是 remake
        最後一個沒做的星圖層——先前卡在「艦隊是單一集合」,多艦隊(第 24 項)做完之後,
        缺的只剩這一層自己的資料。
        ...
        **每個(星, 玩家)一個目標星索引**,−1 = 沒設定。`Draw_Relocation_Links_` 的迴圈就是:
        ...
        - ~~顯示開關 `ShowRelocationLines` 已建,但**還沒有 UI 可以切**~~ ⚠ **2026-08-08 追認:已過期**——第 24 項接進遊戲選單(`cmd/moo2/gamemenu.go:175`)
    ...
    ### **艦隊列表改成真的列「艦隊」+ 清掉 HONEST-STATUS 三條過期斷言**(2026-08-07)。
        ...
        | 「建築集合仍與原版有差(Colony Base、一次性改造沒建模)」 | Colony Base 第 18 項補上;一次性改造第 22 項**查證後確認不該佔格**(原版記旗標那步對那四個編號是跳過的),remake 天然正確 |
    ...
    ### **RELOCATE 鈕的原版語意追出來了:兩段點選 + 四條合法性規則**(2026-08-07,第 24 項留的問題)。


        第 24 項把 RELOCATE 鈕標成「按下去做什麼沒有反組譯確認,所以先不接」。查了符號表,
        整組符號都在,問題直接解掉:
    ...
    ### **`Set_All` / `Clear_All` 集結點 + 遷移連線的顯示開關**(2026-08-07,把第 24/24 項留的小項清掉)。
    ...
    ### **ALL 鈕根本不是集結點**(2026-08-07,把第 24 項的一個推測推翻)。


        ### 兩處手冊,同一句話

        第 24 項寫著「艦隊列表的 ALL 鈕接上 `Set_All_Star_Relocations_`」,並且自己標了「推測」。
...
25. **一星多行星:軌道模型的資料層(第一階段)**(2026-08-07)。
    ...
    **行為逐位元不變**——每顆星仍然只有一顆行星。`TestGeneratedStarsHaveExactlyOneOccupiedOrbit`
    ...
    ### **一星多行星 Step A:33 個呼叫端改走存取器**(2026-08-07,第 25 項的續)。


        第 25 項建好了軌道模型,但所有讀行星的地方仍然直接寫 `s.Planets[星]` ——
    ...
    ### **一星多行星 Step B:同系天體升格成真正的行星**(2026-08-07,task #51 收尾)。
        ...
        第 25 項建了軌道模型、第 25 項把呼叫端改走存取器,這一步把資料真的填進去:
    ...
    ### **同星系多殖民地:拓殖的對象是行星,不是星**(2026-08-07,把第 25/25 項留的最後一步做完)。
    ...
    ### **AI 也會在自己的星系裡拓殖第二顆行星**(2026-08-07,把第 25 項的另一半補上)。


        ### 一個 remake 自己造出來的不對稱

        第 25 項只改了玩家側。`aiExpand` 的候選集寫死成
        ```go
...
26. **人造行星:手冊推翻了 remake 自己的假設**(2026-08-07,建築 48 接線)。
...
27. **是/否確認框 + 一條寫錯的規則**(2026-08-07,把第 24 項留的「沒有 modal 基礎設施」清掉)。
...
28. **戰術格子的獨立戰機單位 + 一個讀錯的欄位**(2026-08-07)。
...
30. **決定性化——網路多人的地基,順手抓出兩個存檔 bug**(2026-08-07)。
    ...
    **抽的種類**都一樣才會落在同一格。`randstream.go` 改成直接騎在 `rand.Source64` 上,
    ...
    ### **傳輸層 + 鎖步協定**(2026-08-07,`internal/netplay`)。
        ...
        那時候已經回推不了是哪一步歪的。每回合比一次(第 30 項的 `StateHash`),
    ...
    ### **連線大廳 + `Choose_Net_Plyrs` 名冊——第一個「尺寸隨資料變」的版面**(2026-08-07)。
        ...
        - **玩家編號由主機指派。** 鎖步的指令排序鍵就是玩家編號(見第 30 項),
    ...
    ### **文字輸入彈窗——原版有一個,而且有自己的 LBX**(2026-08-07)。
        ...
        **`Remapped_Input_Box_Popup_`** ——原版有一個獨立的 modal 彈窗,連自己的 LBX 都有
        ...
        (前兩次:第 30 項的 `Add_Waiting_For_Joiners_Field_` → `Add_Button_Field_`、
        第 30 項的清單列 → `Add_Hidden_Field_`)。
...
31. **TECH LEVEL 的第二個真效果——那張「拿不到所以不臆造」的表挖到了**(2026-08-07)。
    ...
    **AI 一起發**:原版的 `Init_Player_Tech_` 第一個參數就是玩家編號,是逐玩家跑的。
    `TOPIC_NUCLEAR_FISSION`。這是端到端的證據,測試綠不是。
      ...
      ⚠ **2026-08-07 第 44 項補上了**(照抄 `sub_FD335` 的挑選結構),那個函式現在的角色
    - 原版 `byte_199CB5 >= 3` 那條路徑**沒有設 `var_18`**(`enter` 不清堆疊)。

32. **開局建築的優先清單——手冊只給了結論,清單本身在執行檔裡**(2026-08-07)。
...
33. **飛彈速度:那個「手冊自相矛盾」不是矛盾,是漏了條件**(2026-08-07)。
...
34. **地面戰:結構不是「未核實」,是抄了一代的**(2026-08-07)。
    ...
    ### **四種部隊類型是什麼——手冊那一句把三種對上了**(2026-08-07)。
        ...
        第 34 項留下「四種部隊類型沒有對出名字、每種的攻擊力表還沒追」。同一天追完。
    ...
    ### **民兵接上了——`Colony_N_Militia_` 是「除以 5」**(2026-08-07)。
        ...
        第 34 項留下「守方的民兵沒有接:數量公式在 `sub_EC61E`,還沒追」。同一天追完。
    ...
    ### **分兵種接線 + 手冊的四個 hits 數字全部被重建出來**(2026-08-07)。
        ...
        第 36 項留下「Battleoids / 動力裝甲的分兵種接線留給下一輪」。接完了,而且過程中發現
        **手冊列的那四個 hits 值,可以完全由反組譯的加法結構重建**。
        ...
        而第 34 項接上的 `GroundTypeHitsDelta` 是**組成的一部分**。兩個一起用就變成 3 / 4。
...
35. **重力種族特性:High-G 逐字對上,Low-G 的「10%」其實是定值 −10**(2026-08-07)。

    第 34 項留下的加成塊欄位又追出三個,而且三個都能與手冊互證。
...
36. **三張查表讀出來了——十二個科技 id 全部對上,而且 remake 少了一整條通道**(2026-08-07)。
...
37. **聊天列補完:14 則的環、82 byte 的格子、兩種前綴,四個數全是一手的**(2026-08-07)。

    「等待其他玩家」那張畫面的輸入列先前是一條寫著「remake 未實作」的提示帶。
    做得動的理由是第 30 項把文字輸入框做出來了,而這一輪把原版那條線整條讀完:
...
38. **整棵研究樹從二手轉寫升格成一手驗證過**(2026-08-07)。
    ...
    起點是第 36 項刻意延後的 `Choose_Tech_Application_` @ 0xFD335。讀它的過程中
    ...
    ```
    **正好是第 31 項從 `word_18111C` 挖到的六個開局主題**,第三個獨立來源。
    順帶對上第 34 項:`OrigStartingTechs` 裡有 `TECH_PULSE_RIFLE`,而 remake 的
...
39. **三面行星護盾 + 自動實驗室 + 再生反應爐接線**(2026-08-07)。
...
40. **艦員經驗系統:三張加成表早就在,缺的是「經驗怎麼來」**(2026-08-07)。
...
43. **把上一輪自己寫的兩條留白關掉**(2026-08-07)。

    第 42 項的結尾特別強調「**有程式碼消費不等於完整還原**」,並列出仍有寫明的部分實作。
    其中兩條在寫的當下就已經解得開了——擋住它們的東西是我自己前幾輪剛加上去的。

    它一直沒有呼叫端,理由寫在 `session.go` 的註解裡:「因 remake 無多種族人口追蹤,
    第 41 項加上 `ColonyState.UnassimilatedPop` 之後,那個理由就不成立了:
...
44. **先進級開局的 19 個隨機主題:照抄結構,不照抄權重**(2026-08-07)。

    這個缺口從第 31 項就開著,第 88、91 兩次判斷「不照抄」,理由都是同一個。
    這次換個問法:**擋住的到底是哪一部分?**
    ...
    **擋住的只有 `weight` 一項。** 把它一律當 1 之後 `score = horizon ÷ turns`
    ...
    第 38 項挖出來的 `OrigTopicCost`(83 個主題的一手成本)在這裡是燃料——
    - **`weight` 一律 1**,少的是那 985 行的估值。AI 因此不會偏好「對它有用」的科技。
    - `sub_FD335` 尾巴還有一段依 `[player+0x28]`(值 1/2/4/5)的二次過濾。
      那個欄位**沒查到寫入端、沒有名字**——第 37 項在 `Send_Chat_Msg_` 裡也遇到同一個。
    `TOPIC_STARTING_TECH`——第 38 項認出來的那個「自環、永遠不會被解鎖」的容器主題,

45. **上游補完之後,下游要跟著讀真的東西**(2026-08-07)。

    第 44 項把 19 個隨機主題發出去了,但先進級的母星建築**還是只有兩棟**。
...
46. **領袖技能:修掉一條疊加規則,補上四個技能,並確認一個「不是缺口」**(2026-08-07)。
    ...
    ### **環保官:第三次用同一種方式擋錯**(2026-08-08)。
        ...
        第 46 項擋掉農業官/勞工官/科學官,理由是「沒有分項百分比欄位」——第 46 項發現缺的只是三個欄位。
        第 46 項擋掉工程師,理由是「維修速率那個量不存在」——第 46 項發現手冊那條的第二句對得上既有函式。
...
47. **叛亂:同化計時器的另一半**(2026-08-08)。

    第 41 項接了同化,而那個檔的檔頭自己寫著「叛亂系統根本不存在 …
...
49. **三個「待辦」複查:一個早就做完了、一個是證據不足、一個追到源頭**(2026-08-08)。
...
50. **安塔蘭防禦艦隊與 `Calc_Tech_Value_` 兩張資料表解出來了**(2026-08-08)。
    ...
    **外加一座星際要塞**。上限表 `_n_max_antaran_def_ships`(`byte_181746`)逐位元組解出:
    ...
    表③(212 筆 tech-item 記錄)的 topic 欄與第 38 項抽的 `OrigTechTopicTable` 逐筆比對,
    - 我說 `movsd/movsd/movsw` 那段是在複製範本表,agent 查出那三行其實是把字串

51. **軍官畫面座標:openorion2 → 執行檔立即數**(2026-08-08)。

    第 15 項把估計熱區換成 openorion2 的真值座標,那是當時能拿到最好的來源。
...
52. **間諜 UI:那張畫面不存在,所以規劃本身要改**(2026-08-08)。
...
53. **屏障護盾擋生物武器:擋門是「分類的語意」,不是規則本身**(2026-08-08)。

    `internal/gamedata/planetary_shield.go` 的檔尾掛了一條沒接的規則:

    第 50 項從 `Calc_Tech_Value_` 挖出兩張表:`TechItemCategory[212]`(每項科技屬於
    | category | 成員 | 組數 |
    |---|---|---|
...
54. **聊天列:資料模型與畫面都做完了,但字從來沒離開過這台機器**(2026-08-08)。
...
55. **三個「沒有查到寫入端」的欄位,寫入端一直都在**(2026-08-08)。
...
56. **AI 對手的科技,先前完全是偷來的**(2026-08-08)。

    這一項不是從逆向來的,是從**一個探針**來的。第 55 項訂正 `internal/ai/research.go`
...
57. **領袖永久免費——用覆蓋率找出來的**(2026-08-08)。

    第 56 項是靠「這支函式有沒有呼叫端」找到的。那個問法有個明顯的盲點:
...
58. **征服來的人口先前是「免費全額生產」**(2026-08-08)。

    第 57 項用覆蓋率找洞時只看了「應該自動發生」那一類。同一份清單裡還有 196 支
    ...
    remake 有 `UnassimilatedPop`(第 47 項的叛亂系統與多種族士氣懲罰都在用),
...
59. **一個擋門理由過期了三個月都沒人回頭看**(2026-08-08)。
    ...
    這一步靠的是第 55 項的結果:`SpyGovernmentType`、`AssimilationGovernment`、
...
60. **「成就」科技的全帝國效果,四條一條都沒接**(2026-08-08)。

    第 59 項那個形狀又出現一次:**規則寫好了、擋門理由也寫好了,而理由已經不成立**。
...
61. **註解說「打得準也閃得掉」,程式只做了前半**(2026-08-08)。
...
62. **兩個「為了給玩家看」而寫的函式,畫面從來沒用過**(2026-08-08)。
    ...
    | 刻意的偏離 | `PlanetaryShieldEffectiveClimate` | 建築效果不可逆(第 43 項記錄過)|
...
63. **護盾減傷五級裡有四級是錯的,而正確的常數就躺在旁邊**(2026-08-08)。

    第 62 項把「零呼叫端的**函式**」清光了。但那個掃法有個洞——第 60 項的
...
64. **一支偵察艦隊與一支末日之星艦隊,登陸能力完全相同**(2026-08-08)。

    第 63 項修的是「shell 自己編了一個數字,而手冊值就在 gamedata 裡沒人用」。
...
65. **武器傷害全是估計值,而手冊原表一直在可抽文字的 PDF 裡**(2026-08-08)。

    第 63/64 項的形狀是「shell 編了一個數字,手冊值就在旁邊沒人用」。這一項更遠一步:
    **手冊值連抄都沒抄過**,而理由是一句錯的話。
    ...
    **不需要 OCR,也不需要另找手冊。** `moo2_patch1.5/GAME_MANUAL.pdf` 是可直接抽文字的
    ...
    ### **手冊有 18 把武器,remake 只做了 10 把**(2026-08-08)。
        ...
        第 65 項把武器傷害換成手冊真值時,順手記了一句「手冊還列了 remake 沒有的武器
        ...
        | 粒子束 | 10-30 | 15 | 氙素科技 |
        | 重錘裝置 | 100(必中)| 50 | 超維物理 |
        每一項的研究主題都取自 **`gamedata.OrigTechTopic`**——第 50 項從執行檔挖出來、
    ...
    ### **反飛彈火箭:攔截公式抄完了,但沒有一艘船裝得上**(2026-08-08)。
        ...
        | Cost = 70 | **remake 的尺度**(見第 65/65 項),不是手冊值 |
    ...
    ### **p.127 剩下的特殊武器:全部卡在機制,不是卡在資料**(2026-08-08)。
        ...
        第 65–65 項把手冊的武器表逐頁補進來:光束(p.124)、飛彈(p.125)、
        ...
        | 陀螺去穩器 | 1-4 per size class | 資料齊、艦體級數也有了(第 65 項)——但它**不在手冊的球形清單裡**,而 remake 的光束路徑沒有「per size class」這個乘數 |
        `CLAUDE.md` 指名「唯一活來源」是本文件開頭那張「真正還缺的」表,而它 2026-08-07 起

66. **種族只有 7 個自編數字,原版有 31 格特性——一手表挖出來全部換掉**(2026-08-08)。

    第 65 項在活來源表上補了「手冊數值忠實度」那一列,並列出四種還有殘量的問法。
    ...
    ### **布林特性接線:五條寫好的規則終於有呼叫端**(2026-08-08)。
        ...
        第 66 項挖出了 13 族 × 31 格的特性表,把**數值型**那 9 格接了進去;
...
68. **裝甲科技倍率:一則撤回,外加重裝甲與穿甲抵銷**(2026-08-08)。

    ### 先講撤回

    這一輪稍早(第 65 項一帶)拒絕動 `armorHPByName`,理由寫得很篤定:
    | 裝甲 | 手冊原文 | 倍率 |
    |---|---|---|
    ...
    「Heavy Armor **(System)**」,系統要進 `SpecialOptions`,而它不在。與第 67 項的
...
69. **手冊元件完整性盤點 + 飛彈防禦系統家族**(2026-08-08)。
    ...
    | **這一項接上** | 8 | 見下 |
    | **卡機制** | 4 | 陀螺去穩器 / 電漿網 / 停滯力場 / 牽引光束——第 65 項已逐項判定,缺的是戰鬥速度與護盾分面模型 |
    ...
    ### **第 69 項那一桶的第一批,外加修掉第 69 項自己的一個缺失**(2026-08-08)。


        ### 先講缺失

        原因值得記:第 69 項是從 `gamedata/missile.go` 那一側往回找元件的——那個檔案只收
        ...
        寫在 `internal/shell/ship_systems.go` 檔尾,逐項列。摘要:
          - **機制不存在**:保安站(登艦戰,第 61 項)、增強引擎與時間扭曲加速器(戰鬥速度/
    ...
    ### **傷害鏈收成具名結構,順手解掉卡在它後面的兩個系統**(2026-08-08)。
        ...
        第 69 項把結構分析儀與阿基里斯瞄準器列在「要先重構」那一格,理由是:
        ...
        | 阿基里斯瞄準器 | all beam weapons **ignore the target's armor completely** | 與 AP 改造同一個開關 |
          - **飛彈與球形武器沒有跟著收。** 它們各自有不同的判定機制(`ResolveMissileShot` 已經
            在第 69 項收了一個 `MissileDefenses`),硬要三條路共用一個結構會做出一個
          - 第 69 項那一桶還剩 **14 個**,各自的擋門理由寫在 `ship_systems.go` 檔尾。

70. **戰鬥速度與引擎階:一張自我驗證的一手表,外加一個掃描器看不到的硬編值**(2026-08-08)。

    第 69 項那一桶剩下的 14 個,擋門理由聚成幾個共同缺失的機制。最大的一個是
    ...
    **硬編成 1**,所有戰機不論科技多高都跑得一樣慢。它在 `cmd/moo2/tacticalfighter.go`,
      ...
      - **增強引擎**(+5 戰鬥速度)——第 69 項把它列在「機制不存在」那一格,現在成立了。
    ...
    ### **戰術移動不再是瞬移:棋盤比例尺從一手尺寸推出來**(2026-08-08)。
        ...
        第 70 項把戰鬥速度接上了,但移動距離**還是沒有限制**,理由寫得很明白:
    ...
    ### **牽引光束與停滯力場:第 65 項那個「卡在機制」的判定終於解掉**(2026-08-08)。


        第 65 項盤點手冊 p.127 的特殊武器,把這兩個歸進「卡在機制,不是卡在資料」:
...
71. **陀螺去穩器:擋門理由是對的,而正解不是照著它做**(2026-08-08)。

    第 65 項把陀螺去穩器擋在外面,理由寫得很具體:

    那四個目前走 `SpecialOptions`(建造成本)而不是佔格表,先記在 `shipspace.go` 裡,
      - **只有空間壓縮器與陀螺去穩器豁免盾甲**,脈衝星沒有那一句。第 65 項就標過
    ...
    ### **一回合開幾次火:建一次機制,一次解三個元件**(2026-08-08)。
        ...
        第 71 項末尾記著:剩下的 10 個裡有三個卡在**同一個**缺失機制。這一項建那個機制。
    ...
    ### **轟炸機庫:註解說四種,程式碼只有兩種**(2026-08-08)。
        ...
        第 71 項末尾把預期調低了:剩下 6 個不共用機制,要一個一個評估。
...
72. **探針 ③ 的另外半邊:shell 與 cmd 內部函式**(2026-08-08)。

    活表上 ③ 那一格留著一句話:「⚠ **cmd/ 與 shell 內部函式沒掃過**」。
    第 66 項的掃描器只看 `gamedata.X(...)`——**跨套件呼叫**,所以整個 shell/cmd 內部
    從來沒被掃過。第 70 項那個硬編的 `ftlLevel=1` 就是在 cmd/ 撞到的,那是撞運氣。
    ...
    第 69 項加戰鬥掃描器時,我寫了一條 `TestBattleScannerRaisesOnlyAccuracy`。
    它就是基準本身。`ParsecToNormalized` 的調參註解寫的目標是「基礎 2 + 母星星基 2 = 4 parsec」,

73. **盤點:「元件表有」不等於「效果有接」**(2026-08-08)。

    使用者要求盤點剩餘工作。照規矩先查證再回答——結果查證本身變成這一項。

    **隱形裝置在 `SpecialOptions` 裡,而整份程式碼沒有任何地方讀它。**
    最後那一句的語意,就是第 71 項為了超載電容建的 `Fired`。
    | 元件 | 為什麼現在做得了 |
    |---|---|
    | 能量吸收器 | 第 71 項建了跨回合的 `Charged`/`Fired`;手冊指名的例外(位移裝置)第 69 項也在 |
    ...
    ### **三份文件對齊現況**(2026-08-08)。
        ...
        - **快速結算 / 格子戰術** — 戰鬥的兩條路徑。第 69–72 項有五項的缺陷都是
        - **元件表有 ≠ 效果有接** — 第 73 項的教訓,直接收成一個詞條。
    ...
    ### **第二次 review:錯誤斷言的副本比原件多**(2026-08-08)。
        ...
        第 73 項只動了 `WORKLIST.md`。這一輪掃過所有 markdown,發現同一批假斷言在
        | 檔案 | 假斷言 | 事實 |
        |---|---|---|
        | `README.md` | 「行星表面 + 建築 sprite 擺放卡在幾何表尚未抽出」 | 7×7 角點表與 `Cache_Load_Bldg_` 算式都抽出來了 |
        | `README.md` | 「戰機/航母未做」「多 AI 目標選擇為索引順序」 | 兩者都完成(第 28/71 項、`AIEnemyColonyValue`) |
        ...
        | `component-values.md` | 「裝甲缺科技倍率」 | 第 68 項已解 |
    ...
    ### **第三次 review:改用掃描器,不再靠「我記得要查什麼」**(2026-08-08)。
        ...
        **想不到的就查不到**。這一輪換方法。
        ...
        | 檔案 | 假斷言 | 事實 |
        |---|---|---|
        | `tactical-combat-weapon-kinds.md` | 「`WeaponOptions` 沒有任何武器分類到 spherical」 | 第 65/71 項補了三把 |
        | `01-gap-report.md` | 「`ShowRelocationLines` 已建但還沒有 UI 可以切」 | 第 24 項接進遊戲選單 |
        ...
        | `WORKLIST.md` | 「沒有任何一艘船有艦員等級,`shell.Ship` 沒那個欄位」 | `Ship.CrewXP` 在,第 61 項接上攻防兩側 |
        | `moo2-formulas-reference.md` | 「`RaceTrait` 沒有任何函式把數值套進公式」 | 第 66/66 項全接 |
        | `ai-fiscal-solvency.md` | 「`Maintenance` 尚未接線」 | `engine/ai.go:19` 傳進 `decider.ColonyJobs` |
        | `spy-system.md` | 「`interactive.go` 未接間諜畫面/按鈕」 | 第 52 項接進種族關係畫面 |
...
74. **音樂曲目↔場景:不需要人耳,執行檔全寫著**(2026-08-08)。
...
75. **文件政策:過期的斷言直接刪除**(2026-08-08)。
    ...
    **全 repo 一條規則,沒有例外:任何文件都不留錯誤斷言的內容。**
    ...
    正是第 74 項踩到的(少加 `0x10000` 的 object base)。
    ...
    ### **本文的錯誤紀錄清除**(2026-08-08)。
        ...
        依第 75 項的規則,把本文裡 16 處**複述錯誤斷言**的段落刪掉,只留正確結論:

        | 改成 |
        |---|
        | 殖民地畫面的版面與框架美術**都有真值來源**,而且兩者都不在 COLONY.LBX 裡 |
        | 飛彈基地/地面砲台的空間預算模型**早就在**,第 12 項把它當成缺口是漏查 |
        | `Cycle_Ship_Icons_` 不是動畫,而艦隊圖示在原版也不會動 |
    ...
    ### **151 項壓縮:8,163 行 → 3,582 行**(2026-08-08)。
        ...
        第 75 項只是在最前面加了一張規則總表,檔案本身沒有變小。這一項動 151 項本身。
