...
## 75 項速查表(引用裡的括號就是這裡的短標)

| # | 短標 | # | 短標 | # | 短標 |
|---|---|---|---|---|---|
| 1 | 星系生成表 | 26 | 確認框 | 51 | 間諜UI |
| 2 | 一星多行星缺口 | 27 | 戰機中隊單位 | 52 | 生物武器分類 |
| 3 | 殖民地/事件畫面 | 28 | AI遷移非缺口 | 53 | 聊天列幫浦 |
| 4 | 地面戰解算 | 29 | 決定性化 | 54 | 三個寫入端 |
| 5 | 新遊戲設定畫面 | 30 | 科技等級第二效果 | 55 | AI自己研究 |
| 6 | 殖民地畫面框架 | 31 | 開局建築清單 | 56 | 領袖維護費 |
| 7 | 曲速前FTL限制 | 32 | 飛彈速度 | 57 | 征服人口產出 |
| 8 | 行星表面調查 | 33 | 地面戰結構 | 58 | 間諜三項加成 |
| 9 | 星圖艦隊圖示 | 34 | 重力種族特性 | 59 | 成就科技效果 |
| 10 | 蟲洞 | 35 | 三張查表 | 60 | 艦員防禦 |
| 11 | 48棟建築盤點 | 36 | 聊天列資料 | 61 | 兩個UI函式 |
| 12 | 四大系統缺口盤點 | 37 | 研究樹一手驗證 | 62 | 護盾減傷 |
| 13 | 指揮點數視窗 | 38 | 行星護盾等三棟 | 63 | 陸戰隊運力 |
| 14 | 地表道路 | 39 | 艦員經驗 | 64 | 武器傷害真表 |
| 15 | 星雲 | 40 | 同化系統 | 65 | 種族特性31格 |
| 16 | 秒差距模型 | 41 | 戰機基地 | 66 | 高能聚焦 |
| 17 | 拓殖基地 | 42 | 關掉兩條留白 | 67 | 裝甲科技倍率 |
| 18 | 銀河貨幣交易所 | 43 | 先進級開局主題 | 68 | 元件盤點+飛彈防禦 |
| 19 | AI請求會談 | 44 | 下游讀真值 | 69 | 戰鬥速度與引擎階 |
| 20 | 手冊搜尋假陰性 | 45 | 領袖技能 | 70 | 陀螺去穩器 |
| 21 | 改造不佔格 | 46 | 叛亂 | 71 | 探針③內部函式 |
| 22 | 黑洞動畫 | 47 | AI艦隊移動 | 72 | 元件表有≠效果有接 |
| 23 | 多艦隊模型 | 48 | 三個待辦複查 | 73 | 音樂場景表 |
| 24 | 軌道資料層 | 49 | 安塔蘭艦隊+科技估值 | 74 | 文件政策 |
| 25 | 人造行星 | 50 | 軍官畫面座標 | 75 | 編號壓縮 |
...
| 12 | 多人連線 11 個畫面 | — | ✅ `MP_Setup`(`cmd/moo2/multiplayer.go`)與 `Hotseat`(`cmd/moo2/hotseat.go`)2026-08-07 已建,版面座標取自反組譯(見下方第 3 項(殖民地/事件畫面))。`Net_Next_Turn`(第 29 項(決定性化))與 `Choose_Net_Plyrs`(第 29 項(決定性化))2026-08-07 已建。`Modem_Setup`/`NullModem_Setup`/`Comm Info` **不做**(硬體已不存在)。`Join_Net`/`Generic_Net_Info`/`SendGet_Net_Info` 是同一張畫面的不同狀態(第 29 項(決定性化)),`Choose_Multi_Net_Game` 見第 29 項(決定性化)。**11 張全部結案:8 做 / 3 不做** |
...
| 遷移連線的顯示開關沒有 UI | ✅ 第 23 項(⚠ 那一列不是原版版面,原版有一整個設定畫面) |
| `Clear_All` 集結點沒有 UI 入口 | ✅ 第 23 項(多艦隊模型)。⚠ 同時訂正——**ALL 鈕不是 Set_All**,手冊兩處明說它是「全選/全不選艦艇」 |
| 遷移確認框 | ✅ 第 26 項(版面取自 `Confirmation_Box_`)。⚠ 同時訂正——原版那個條件是**怪獸**不是艦隊 |
...
| 多艦隊模型 | ✅ 第 23 項(資料層)+ 第 23 項(艦隊列表)+ 第 23 項(分艦隊) |
...
## 二、逐畫面重建與子系統補齊(2026-08-07)
    ...
    | 畫面 | 狀態 |
    |---|---|
    | 地面戰 | ✅ 2026-08-07(第一個實例,見第 3 項(殖民地/事件畫面))|
    | 軌道轟炸 | ✅ 2026-08-07(第 3 項(殖民地/事件畫面))|
    | NEW GAME 設定 | ✅ 2026-08-07(第 5 項(新遊戲設定畫面),順便修掉 PLAYERS 被當 RACE 用)|
    | 殖民地 | ✅ 2026-08-07(第 6 項(殖民地畫面框架),框架 = COLPUPS.LBX#5)|
    | 種族選擇 | ✅ 2026-08-07(第 5 項(新遊戲設定畫面),順便修掉左右擺反)|
    | 艦艇設計 | ✅ 2026-08-07(第 5 項(新遊戲設定畫面),六格不等距;右上兩個面板待追)|
    | 多人設定 / 熱座交接 | ✅ 2026-08-07(第 3 項(殖民地/事件畫面))|
...
7. **TECH LEVEL 接上第一個 gameplay 效果:曲速前的 FTL 限制**(2026-08-07)。
    ...
    第 5 項(新遊戲設定畫面)留下的「TECH LEVEL 只存設定、不影響 gameplay」不再完全成立。手冊直引
...
37. **整棵研究樹從二手轉寫升格成一手驗證過**(2026-08-07)。
    ...
    ```
    **正好是第 30 項(科技等級第二效果)從 `word_18111C` 挖到的六個開局主題**,第三個獨立來源。
...
43. **先進級開局的 19 個隨機主題:照抄結構,不照抄權重**(2026-08-07)。
    ...
    這個缺口從第 30 項(科技等級第二效果)就開著,第 88、91 兩次判斷「不照抄」,理由都是同一個。
    這次換個問法:**擋住的到底是哪一部分?**
    ...
    **擋住的只有 `weight` 一項。** 把它一律當 1 之後 `score = horizon ÷ turns`
    ...
    - `sub_FD335` 尾巴還有一段依 `[player+0x28]`(值 1/2/4/5)的二次過濾。
      那個欄位**沒查到寫入端、沒有名字**——第 36 項(聊天列資料)在 `Send_Chat_Msg_` 裡也遇到同一個。
...
52. **屏障護盾擋生物武器:擋門是「分類的語意」,不是規則本身**(2026-08-08)。
    ...
    第 49 項(安塔蘭艦隊+科技估值)從 `Calc_Tech_Value_` 挖出兩張表:`TechItemCategory[212]`(每項科技屬於
    | category | 成員 | 組數 |
    |---|---|---|
...
56. **領袖永久免費——用覆蓋率找出來的**(2026-08-08)。
    ...
    第 55 項(AI自己研究)是靠「這支函式有沒有呼叫端」找到的。那個問法有個明顯的盲點:
...
57. **征服來的人口先前是「免費全額生產」**(2026-08-08)。
    ...
    第 56 項(領袖維護費)用覆蓋率找洞時只看了「應該自動發生」那一類。同一份清單裡還有 196 支
...
59. **「成就」科技的全帝國效果,四條一條都沒接**(2026-08-08)。
    ...
    第 58 項(間諜三項加成)那個形狀又出現一次:**規則寫好了、擋門理由也寫好了,而理由已經不成立**。
...
62. **護盾減傷五級裡有四級是錯的,而正確的常數就躺在旁邊**(2026-08-08)。
    ...
    第 61 項(兩個UI函式)把「零呼叫端的**函式**」清光了。但那個掃法有個洞——第 59 項(成就科技效果)的
...
64. **武器傷害全是估計值,而手冊原表一直在可抽文字的 PDF 裡**(2026-08-08)。
    ...
    **不需要 OCR,也不需要另找手冊。** `moo2_patch1.5/GAME_MANUAL.pdf` 是可直接抽文字的
    ...
    ### **手冊有 18 把武器,remake 只做了 10 把**(2026-08-08)。
        ...
        | 粒子束 | 10-30 | 15 | 氙素科技 |
        | 重錘裝置 | 100(必中)| 50 | 超維物理 |
        每一項的研究主題都取自 **`gamedata.OrigTechTopic`**——第 49 項(安塔蘭艦隊+科技估值)從執行檔挖出來、
...
68. **手冊元件完整性盤點 + 飛彈防禦系統家族**(2026-08-08)。
    ...
    ### **第 68 項(元件盤點+飛彈防禦)那一桶的第一批,外加修掉第 68 項(元件盤點+飛彈防禦)自己的一個缺失**(2026-08-08)。
        ...
        寫在 `internal/shell/ship_systems.go` 檔尾,逐項列。摘要:
          - **機制不存在**:保安站(登艦戰,第 60 項(艦員防禦))、增強引擎與時間扭曲加速器(戰鬥速度/
    ...
    **隱形裝置在 `SpecialOptions` 裡,而整份程式碼沒有任何地方讀它。**
    ...
    ### **第三次 review:改用掃描器,不再靠「我記得要查什麼」**(2026-08-08)。
        ...
        **想不到的就查不到**。這一輪換方法。
        ...
        | `WORKLIST.md` | 「沒有任何一艘船有艦員等級,`shell.Ship` 沒那個欄位」 | `Ship.CrewXP` 在,第 60 項(艦員防禦)接上攻防兩側 |
