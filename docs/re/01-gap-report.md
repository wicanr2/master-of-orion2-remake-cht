...
| **星圖:遷移連線**(4 層裡剩的那 1 層)| 資料模型 | 蟲洞(35)、星雲(44)、星門(47)、外交燈號(50)已打通。**多艦隊模型第 56 項已完成**,剩下的前置是「逐殖民地造艦佇列 + 集結點」——要畫的是「從哪個殖民地生產、送到哪」那條線 |
...
| ~~**鍵盤快捷鍵**~~ | ✅ | 第 55 項全數接完(F1/F2、F5/F6、F9、F10、ALT+F9)。**ALT+F1..F8 的設定開關仍不碰**——PDF 邊欄標籤抽出來有 off-by-one 風險,要先對原版逐項確認 |
