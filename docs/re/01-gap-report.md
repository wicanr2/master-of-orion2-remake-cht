...
> | 問法 | 抓到的例子 | 殘量 |
> |---|---|---|
> | ① 哪些函式**從來沒被執行過**(一局 300 回合的覆蓋率) | 領袖維護費、征服人口 3/4 產出 | 36 支已逐支分類完 |
> | ② 哪些匯出常數**零消費** | 護盾減傷(五級錯四級) | 190 個,多數是原版列舉鏡像 |
> | ③ 哪些參數**被餵固定值** | 飛彈閃避恆傳 0、`hasAMR` 恆 false | 未系統化掃過 |
> | ④ 手冊有而 remake **沒抄的資料** | 武器傷害全表、陸戰隊運力、8+4+2 項武器 | p.126-127 的其餘特殊武器(見下) |
>
> **①②** 已挖到見底,**③④** 還有。③ 特別值得做:它抓的是「接了但等於沒接」,
> 而那種洞在測試與截圖上都看不出來。
>
