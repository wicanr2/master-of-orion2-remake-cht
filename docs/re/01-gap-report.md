...
77. **原版元件表挖到了,五個「擋著」的元件同一輪全部接完**(2026-08-08)。
    ...
    **「Resolve\* 有這個參數」不等於「呼叫端有填」。** 結構欄位漏填拿到的是零值,不會編譯
    ...
    - **主動權一律當成開著**,所以走手冊「accumulates until it is fired」那條,儲能不每回合歸零
      ——remake 的齊射次序本來就照手冊的主動權公式排。
...
85. **英文版面撞牆:每修一層就冒出下一層,而每一層只有截圖看得到**(2026-08-08)。
    ...
    量過成本:**420 個繪製呼叫點**(50 `DrawImage` / 158 `vector.*` / 198 文字)
    加上命中區映射。那是獨立一輪,不是這一輪順手做的事。**沒做,不假裝做了。**
