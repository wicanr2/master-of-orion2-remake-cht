...
19. Smacker 過場 → **解碼器已寫,尚未解對,故未接進遊戲**(2026-08-07,`internal/smk/`)。

    先釐清一個前提:MOO2 的片頭與各結局過場**不是 LBX**,是**裸的 Smacker 檔**,
    只是沿用了 `.LBX` 副檔名——`INTRO.LBX` 開頭四個位元組就是 `SMK2`(480×160、1407 幀、
    ≈13 fps),`WININFIN` / `LOSERFIN` / `ORIONFIN` / `ANATKFIN` / `AMEBAFIN` / `PLNTDFIN` /
    `DIMTVFIN` / `ANWINFIN` 同理。所以這一項的前置不是找檔案,是寫一個 Smacker 解碼器。

    **已驗證正確**(INTRO / ORIONFIN / WININFIN 三個檔都成立):
    - 標頭、幀大小表、幀旗標表、樹區的解析——所有幀資料吃完後檔案**殘餘 0 位元組**。
      幀邊界只要錯一個位元組這個數字就不會是 0。
    - 四棵 Huffman 樹的節點數都在標頭上界 `((size+3)>>2)+4` 之內。
    - 全片 1407 幀解得完,**沒有任何一次位元流走出樹外**。
    - 畫面內容連貫:認得出飛船、太空站、行星地表、星雲,顏色也對。

    **尚未解對**:1407 幀裡 **162 幀**(全是 10–14 KB 的大幀)多讀了位元(最多一幀 5952 bits);
    小幀精準到只差 −9 bits(正常的位元組尾端補齊)。多讀的部分拿到 0,解出零星壞塊。

    **已測過、確定不是原因的**(負面結果,別再重試):escape 位元組序反轉(162→701,更糟)、
    MRU 拿掉 `v != last[0]` 判斷(162→210,更糟)、對 SMK2 也讀 SMK4 的全彩模式位元
    (162→517,更糟)。問題已收斂到「大幀的區塊迴圈裡某條路徑比編碼器多消耗位元」。

    診斷工具在 `cmd/smkdump`:`SMKDUMP_USAGE=1` 印每幀位元消耗與超讀統計,
    `SMKDUMP_DIAG=1` 另外輸出索引灰階圖與調色盤色卡(索引圖乾淨而彩色圖髒 = 調色盤問題,
    反之 = 影像解碼問題)。真檔驗收:`MOO2_SMK_TEST=<某個 .smk> scripts/test.sh ./internal/smk/`。

    **刻意不接進遊戲**:一段帶明顯雜訊的片頭比沒有片頭更糟。
