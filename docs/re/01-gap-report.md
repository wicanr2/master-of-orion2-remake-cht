...
| **一星多行星(含建築 48 人造行星)** | 資料模型 | **第一階段已完成**(第 61 項:軌道模型 + 存取器 + 存檔遷移,行為不變)。剩第二階段:把 `SystemBodies` 升格成真正的行星、填滿軌道表,再讓殖民/前哨站/建築 48 以「星系裡的某一顆行星」為對象(約 33 個呼叫端) |
...
| `Set_All` / `Clear_All` 集結點 | 未接 | 艦隊列表的 ALL 鈕多半是它們(第 59 項),尚未確認也尚未接 |
