...
>    第 148 項訂下的分界——**其餘所有文件只留現況,錯誤推導的全文只留在這裡**。
>
...
38. **飛彈基地/地面砲台其實早就算得出來——是接線漏了,不是模型缺了**(2026-08-07,第 37 項的訂正)。
    ...
    飛彈基地與地面砲台的空間預算模型**早就在**,第 37 項把它當成缺口是漏查:
...
108. **安塔蘭防禦艦隊與 `Calc_Tech_Value_` 兩張資料表解出來了**(2026-08-08)。
...
111. **屏障護盾擋生物武器:擋門是「分類的語意」,不是規則本身**(2026-08-08)。
    ...
    缺的不是規則(手冊寫得很完整),是**「哪些科技算生物武器」這份名單**——
    而那份名單一直在執行檔裡。
...
143. **盤點:「元件表有」不等於「效果有接」**(2026-08-08)。
    ...
    **「元件表有」與「效果有接」是兩件事,而先前沒有任何一種問法分得出來。**
    ...
    | 元件 | 為什麼現在做得了 |
    |---|---|
    | 能量吸收器 | 第 140 項建了跨回合的 `Charged`/`Fired`;手冊指名的例外(位移裝置)第 133 項也在 |
    | 戰鬥艙 | `ShipHullSpace` 與 `ShipDesignSpaceUsed` 都在 |
    | 相位匿蹤 | 只需要「不可被選為目標」一個旗標 + 回合數(`t.round` 已有) |

    第二列與第 142 項的掃描範圍同屬一類:**擋門理由可能從寫下的那天就不成立**,不只是會過期——所以「查不到所以標近似」的前提是真的查過。
    ...
    **格子戰術傳的是真距離**,所以測距瞄準器做得了(快速結算固定 range=2,那條路徑上無效)。
...
148. **改政策:過期的斷言直接刪掉,不留刪節線**(2026-08-08)。
    ...
    ⚠ 這一項當時把本文列為例外(「錯誤推導的全文只留在這裡」)。
    **第 150 項取消了那個例外**——同一條規則套用到全 repo,包括本文。
...
149. **本文不是現況了:活表併進 WORKLIST**(2026-08-08)。

    使用者:「`01-gap-report.md` 如果沒有保留的價值就刪除。」
    ...
    |---|---|
    | 規模 | 8,154 行 / 470 KB |
    | RE 硬資料 | **196 個十六進位位址、131 列表格** |
    | 被引用 | 18 個檔案、41 處(含 `CLAUDE.md`、`README.md`、`CONTEXT.md`) |

    那 196 個位址是一次性成本的知識——`Play_Background_Music_ @0x2484F`、行星表面 7×7 角點表、
    種族特性換算表、`stride 0xEA9`、VA→檔案位移 `0x7E694`、戰鬥速度表、`Assign_Combat_Grids_`
    的 81×68。每一項重挖都是數小時起跳,刪掉等於把它們丟回未知。

    **但直覺指到的問題是真的**:第 144 項把剩餘工作表搬進 `WORKLIST.md` 之後,
    **兩份活表並存了四輪**——而「複製出來的每一份都會過期」正是本文自己反覆寫的教訓。
    本文的 ④ 殘量在 27 行裡出現,WORKLIST 的在 8 行裡出現,兩邊都是「現況」。

    ### 處置:拆職責,不刪檔案

    - 本文開頭那 85 行的活表**刪除**,換成一個指標。
    - `CLAUDE.md` / `CONTEXT.md` / `HANDOFF.md` / 五份 kickoff 的「唯一活來源」指標
      全部改指 `WORKLIST.md`。
    - 本文的職責寫死成兩件,**都不是現況**:RE 硬資料、工程日誌。

    ### 日誌那半的處置

    當時的判斷是「錯誤推導的全文只留在這裡」。**第 150 項改了**:
    複述的錯誤斷言全部刪除,只留正確結論與**正面敘述的教訓**——
    擋門理由會過期也可能當初就寫錯、元件表有不等於效果有接、
    位址少加一個 object base 會讓三種掃描一起說謊。
    這些形狀不需要把錯誤原文抄一遍也講得清楚。

    無程式碼改動,截圖無差異。

150. **取消例外:本文也不留錯誤紀錄**(2026-08-08)。

    使用者:「修正 gap-report,不要留下錯誤的紀錄。」

    第 148 項把全 repo 的過期斷言刪光時,把本文列為例外,理由是「它是工程日誌,
    當初怎麼推錯的正是它的內容」。第 149 項又重申了一次。**那個例外現在取消。**

    ### 刪掉的是什麼

    16 處**複述錯誤斷言**的段落——「`X.go` 原本寫著『……』,兩句都不對」這種形狀。
    刪掉引號裡的內容,只留正確結論:

    | 原本的寫法 | 改成 |
    |---|---|
    | 「`colonyscreen.go` 原本寫著『沒有原版版面資料…』,兩句都不對」 | 「版面與框架美術都有真值來源,而且兩者都不在 COLONY.LBX 裡」 |
    | 「第 37 項寫『…算不出固定值』。那句話是錯的」 | 「空間預算模型早就在,第 37 項把它當成缺口是漏查」 |
    | 「`shipicon.go` 檔頭原本寫著『8 幀在跑動畫』,兩句都錯」 | 「`Cycle_Ship_Icons_` 不是動畫,而艦隊圖示在原版也不會動」 |
    | 「`antaran_victory.go` 原本寫著:> 保守預設 6 艘…」 | 「數字執行檔裡有,而且組成不是同級的」 |
    | 「舊理由」欄 | 「為什麼現在做得了」欄 |

    ### 保留的是什麼

    - **正確結論與 RE 硬資料**(196 個位址、131 列表格)——本文存在的主要理由。
    - **「這件事做完了」的完成標記**(`~~題目~~ ✅ 已建)——那不是錯誤紀錄。
    - **教訓,一律正面敘述**。例如不再寫「那句話不是過期,是從寫下的那一天就錯的」,
      改寫成規則本身:**擋門理由可能從寫下的那天就不成立,所以「查不到所以標近似」
      的前提是真的查過。** 教訓講得清楚,不必把錯誤原文抄一遍。
    - **標題裡的「X 不是 Y」**(如「index >= 0xF0 從來不是顏色」)——那陳述的是正確事實,
      只是用對比的句式,不是錯誤紀錄。

    ### 現在全 repo 一條規則

    **任何文件都不留錯誤斷言的內容,包括本文。** 要知道某項當初怎麼錯的,看 git log。
