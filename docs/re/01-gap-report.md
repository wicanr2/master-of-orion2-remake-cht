...
149. **本文不是現況了:活表併進 WORKLIST**(2026-08-08)。
    ...
    **兩份活表並存了四輪**——而「複製出來的每一份都會過期」正是本文自己反覆寫的教訓。
    ...
    第 148 項訂的分界是「其餘所有文件只留現況,錯誤推導的全文只留在這裡」。
    如果連這裡也刪,那個分界就沒有落點——「當初怎麼推錯的」會整個消失。
    而這一整輪(第 111–148 項)最有價值的產出恰恰不是任何一項的內容,
    是**那些錯誤共有的形狀**:擋門理由會過期、也可能當初就寫錯;
    元件表有不等於效果有接;位址少加一個 object base 會讓三種掃描一起說謊。
    那些形狀只有在錯誤的全文還在的時候才看得出來。
