...
3. ~~獨立 Colony 畫面 + Event 畫面~~ → **兩個都已完成**(2026-08-06)。
    ...
    ### 多人連線與熱座（現代傳輸轉接）
        ...
        **熱座的席位模型**:原版帝國資料本來就是 `player[i]` 陣列(stride 0xEA9)+ 當前索引
        ...
        ⚠ 誠實留白(此處摘要):
        ② 2026-08-27 以 IDA Pro 9.4 重查後，`sub_628E2 @ 0x628E2..0x62BB7` 可證實為熱座
        互動流程、置中兩個已載入物件並讀取文字 ID `0xA2..0xA5`，但不能證明 remake 的逐回合
        privacy gate、視窗尺寸、`+0x0E/+0x46` 錨點或 TAKE OVER 文案是原版精確畫面；舊尺寸／
        錨點斷言已撤回。現行 360×230 全畫面遮蔽視窗是明標的同機隱私 adapter，證據見
        `docs/re/hotseat-handoff-ui-audit-20260827.md`。
...
87. **戰術控制列:七顆鈕翻譯做完了,熱區一個都沒有**(2026-08-08)。
    ...
    | 選項 | ⚠ 原版開啟設定畫面；remake 的 13 列設定頁已完成，但戰鬥中保存畫面狀態並返回同一戰局的轉場尚未接線，按鈕會顯示這項準確限制 |

    原本的「後三顆點下去會說明自己為什麼沒有反應」只剩選項鈕成立；等待/完成已改為真正改變逐艦行動狀態。2026-08-26 再稽核時也刪除了「設定畫面尚未完成」的過期斷言；缺口只限於戰術畫面的保存／返回轉場。
...
## 106. NPC 特殊宣戰理由 20／68／113（2026-08-28）

IDA Pro 9.4 對 `sub_25DF1 @ 0x25DF1` 的 raw 指令重新核對後，已接回 raw government 3
的 reason 20、方向關係門檻 reason 68，以及食物赤字 reason 113。`sub_4DAB2 @ 0x4DAB2`
證實 `player+0xB0` 為負時遞增 `+0x7EC`、非負時歸零；remake 現由 AI 經濟的
