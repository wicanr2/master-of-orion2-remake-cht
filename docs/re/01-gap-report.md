...
| 缺口 | 性質 | 備註 |
|---|---|---|
| **網路多人** | 整塊子系統 | ~~決定性化~~(72)、~~傳輸層 + 鎖步~~(73)、~~指令解譯器~~(74)、~~`Net_Next_Turn` 等待畫面~~(75)、~~連線大廳 + `Choose_Net_Plyrs` 名冊~~(76)、~~連線狀態面板 7 狀態~~(77)、~~區網探索 + `Choose_Multi_Net_Game`~~(78)、~~文字輸入彈窗~~(79)。**整塊完成**:傳輸層 + 鎖步 + 決定性 + 指令層 + 大廳 + 區網探索 + 6 張畫面 + 輸入框;3 張明確不做(硬體已不存在)。剩**聊天列**(加分項,`Chat_Box_Input_Loop_` @ 0xF55A4 已定位);`Modem_Setup` / `NullModem_Setup` / `Comm Info` 三張**不做**(數據機與序列線硬體已不存在,remake 走 TCP——替不存在的硬體做設定畫面不是還原) |
...
80. **TECH LEVEL 的第二個真效果——那張「拿不到所以不臆造」的表挖到了**(2026-08-07)。
    ...
    **AI 一起發**:原版的 `Init_Player_Tech_` 第一個參數就是玩家編號,是逐玩家跑的。
    ...
    - 先進級在固定表之外**還會隨機送 19 個**,remake 只發固定表的 6 個。
      缺口大小由 `gamedata.StartingTopicRandomExtras` 回報——**讓缺口是一個看得見的數字,
      不是註解裡的一句話**。要接得先有「隨機挑一個尚未取得的主題」的決定性亂數流。
...
91. **整棵研究樹從二手轉寫升格成一手驗證過**(2026-08-07)。
    ...
    **正好是第 80 項從 `word_18111C` 挖到的六個開局主題**,第三個獨立來源。
    ...
    照抄風險太高」,這次量到了數字,結論不變。先進級開局的 19 個隨機主題仍然沒接
...
96. **征服人口的同化系統**(2026-08-07)。
    ...
    **`colonyMoralePercent` 根本沒有讀它**——那個建築名在整個 repo 裡只出現在資料表與註解裡。
    ...
    41 棟裡名字從未被程式碼消費的剩 **4 棟**:戰機基地(格子戰鬥的獨立戰機單位)、
    恆星轉換器,以及先前已知的兩棟。
