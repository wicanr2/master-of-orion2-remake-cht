...
> | ③ 哪些參數**被餵固定值** | 種族特性整叢(第 129 項)、`apNegated`、`hefBonus`、`ftlLevel`(第 136 項)、硬化護盾 + 攻方掃描器(第 142 項) | **兩側都掃過了**(第 142 項用 go/ast 掃 shell + cmd 內部函式,17 條線索)。gamedata 那側剩 5 小項;內部函式那側剩 4 條**已判定為正確**的(`ResolveShot(armorPiercing)`/`ResolveShotWithMods` 是被 `ResolveBeamShot` 取代的舊包裝、`hasECCM` 擋在飛彈 mod 後面、軌道轟炸的 `hardShield=false` 是對的——打的是行星不是船)|
> | ④ 手冊有而 remake **沒抄的資料** | 武器傷害全表、裝甲倍率階梯、飛彈防禦家族 | **3 真擋 + 5 待做**,見下方「④ 的準確殘量」(第 143 項重新盤點;「剩 6 個」那個數字是錯的)|
>
...
>
> ### ④ 的準確殘量(2026-08-08 第 143 項重新盤點)
>
> 上一版寫「剩 6 個元件,各自擋在不同的缺席系統後面」。重新逐項查證之後,**那個數字與那句話都不準**。
>
> **A. 真的還擋著(缺前置系統,不是缺數字)——3 項**
>
> | 元件 | 擋在什麼後面 |
> |---|---|
> | 保安站 | 登艦戰不存在(第 119 項) |
> | 傳送器 | 同上,另需護盾分面 |
> | 匿蹤力場 | 「completely invisible on the Galaxy Map」——remake 的 AI 艦隊是抽象戰力、沒有地圖座標,玩家自己的艦隊對玩家永遠可見。**沒有「敵方看得到我的艦隊」這件事可以隱藏** |
>
> **B. 擋門理由已過期或本來就錯——5 項(下一輪的料)**
>
> | 元件 | 舊理由 | 為什麼不成立 |
> |---|---|---|
> | 能量吸收器 | 「需儲能狀態」 | 第 140 項已建跨回合的 `Charged`/`Fired`;位移裝置(手冊指名的例外)第 133 項也在 |
> | 戰鬥艙 | 「remake 沒有逐元件佔格的造艦模型」 | **錯的**:`gamedata.ShipHullSpace` + `shell.ShipDesignSpaceUsed` 都在 |
> | 相位匿蹤 | 「需戰鬥可見性」 | 只需要**不可被選為目標**一個旗標 + 回合數(`t.round` 已有) |
> | 測距瞄準器 | 「快速結算固定 range=2」 | 只對一半——格子戰術傳的是真距離 `dist`。⚠ **這一項先前不在任何缺口名單上**(第 133 項的盤點漏了它) |
> | 隱形裝置 | (沒有理由,因為沒人發現) | ⚠ **已經在 `SpecialOptions` 裡,但整份程式碼沒有任何地方讀它**——裝得上、花得了錢、不做任何事。手冊規則完整:未攻擊時 +80 光束防禦、飛彈與魚雷 50% 未命中,開火即失效,整整一回合沒開火才能重新隱形(**正是第 140 項 `Fired` 的語意**) |
>
> ⚠ **這次盤點本身的方法值得記**:先前所有盤點問的都是「**元件表裡有沒有這一項**」,
> 而隱形裝置證明那個問法有洞——**「元件表有」與「效果有接」是兩件事**。
> 新的問法是掃「元件名在表以外的程式碼裡出現幾次」,零次就是裝飾品。
> 32 個特殊元件跑出來兩個:隱形裝置(真漏)與重生程序(已標註的種族特性代理,非漏接)。
>
> ⚠ 另外,`internal/shell/ship_systems.go` 檔尾那份「沒有接的與理由」清單,
> 這次重寫時發現**上一版 12 項裡有七項早就接掉了**(第 134–141 項),理由卻還留著。
> **那份清單自己就是它警告的那種東西。**
>
> ⚠ **第 142 項找到第八次,而且是更糟的一種**:那句擋門理由**從寫下的那一天就是錯的**。
> 「手冊沒有公開逐科技的 parsec 數字」——手冊三個掃描科技的條目各自明寫著自己的數字,
> 就在同一份 PDF 裡。**「查不到所以標近似」是誠實的做法,前提是真的查過。**
> 過期的斷言至少曾經是對的;沒查就寫的斷言連那一點都沒有,而且長得一模一樣。
>
...
> ⚠ **2026-08-08 追認:三項裡兩項已解,只剩艦艇設計右上那兩格。**
> - 行星表面 + 建築 sprite:7×7 角點表(第 27 項)與**建築編號 → 圖檔算式**
>   (`Cache_Load_Bldg_` @ 0xAF6DC)都抽出來了,見 `cmd/moo2/colonysurface.go`。
> - 戰機/航母:中隊在戰術格子上已是獨立單位(第 69 項),三型戰機庫齊備(第 141 項)。
...
140. **一回合開幾次火:建一次機制,一次解三個元件**(2026-08-08)。

    第 139 項末尾記著:剩下的 10 個裡有三個卡在**同一個**缺失機制。這一項建那個機制。
    ...
    這是第 136(速度)/137(移動)/138(狀態)之後,同一種「先建機制再放資料」的第四次。

    ### 冷卻的讀法:手冊寫得比想像中嚴

        超載電容:  cannot be fired twice in a turn again until they have spent at least
                   1 full turn **UNUSED**
        快速飛彈架:until it has allowed them to **remain unused** for 1 turn

    **「unused」不是「沒有連射」,是「完全沒開火」。** 兩條都用同一個字。
    所以充能的條件是「整整一回合沒有開火」——**連射之後再單射,仍然沒有充到電**。

    這個差別在對戰裡很明顯:玩家不能靠「連射 → 單射 → 連射」無限循環。
    照字面實作反而比「連射後隔一回合」更嚴,而字面就是規則。

    時間扭曲加速器**沒有冷卻**:手冊說「the ship gets two combat rounds for every one that
    normal ships get」——那是持續的,不是充放電。三個系統共用一個機制,但**不共用一套規則**。
...
142. **探針 ③ 的另外半邊:shell 與 cmd 內部函式**(2026-08-08)。

    活表上 ③ 那一格留著一句話:「⚠ **cmd/ 與 shell 內部函式沒掃過**」。
    第 129 項的掃描器只看 `gamedata.X(...)`——**跨套件呼叫**,所以整個 shell/cmd 內部
    從來沒被掃過。第 136 項那個硬編的 `ftlLevel=1` 就是在 cmd/ 撞到的,那是撞運氣。

    ### 這次改用 Go 自己的 parser

    正規表示式在跨套件那一輪就已經很勉強(第 129 項為了巢狀括號重寫過一次)。
    內部函式還多了方法呼叫、多行引數、同名歧義,所以這次直接 `go/ast`:
    收所有函式宣告 → 收所有非測試檔的呼叫端 → 逐參數位置看是不是每個呼叫端都給同一個字面值。
    ...
    17 條裡 13 條是對的(舊包裝、只有一個合法值的參數、擋在缺席系統後面的旗標)。
    剩下四條全部指向同一件事:**元件或科技的第二個效果沒接**。
    ...
    第四列最值得記。`gamedata/detection.go` 的檔頭寫著:

    > ⚠ 全面近似(誠實標註):原版 MOO2 手冊只定性描述「掃描科技越高階看得越遠」,
    > 並未公開逐科技的 parsec 數字

    **那句話不是過期,是從寫下的那一天就錯的。** 三個掃描科技的手冊條目各自明寫著
    自己的 parsec 值,就在同一份 PDF 裡,`grep "^Neutron Scanner"` 一次就看得到。

    而且自編值把**順序也弄反了**:那版按科技樹階序挑「最高階」,迅子排在中子後面,
    於是研究出迅子會把更遠的中子覆蓋掉——**能力不進反退**。手冊說中子 6 比迅子 4 遠。

    這一組先前**零測試覆蓋**。一個沒有任何測試看著的查表函式,寫錯不會有任何徵兆。

    ### 順手撞到的一個真 bug:選單狀態洩漏進遊戲規則

    接戰鬥掃描器的 +2 parsec 時要碰艦隊那一側,才看到 `VisibleStars()` 傳的是
    `s.Fleet().AtStar`——**目前選中的那一支**艦隊。多艦隊模型上線後這就變成 bug:
    **切換選中的艦隊會改變戰爭迷霧**。「選中哪一支」是選單狀態,不該影響遊戲規則。
    現在每支艦隊都是偵測源,測試直接釘住「切換選中不改變任何一顆星的可見性」。

    ### 一條測試把錯的理解釘住了

    第 134 項加戰鬥掃描器時,我寫了一條 `TestBattleScannerRaisesOnlyAccuracy`。
    手冊那一段有**兩句話**,我接了第一句,然後用測試名稱裡的「Only」把它封裝成事實。
    **測試名稱裡的「只」是一種斷言**,而這一條斷的是我當時的理解,不是手冊。
    (同樣的坑第 133 項慣性穩定器剛踩過——一個元件兩個效果、只接一半。連續兩項。)

    ### 截圖差異全部來自哪裡:用實驗確定,不用推理

    畫廊六張有差,含 `30_netwait` 的**狀態指紋**——指紋變代表持久化狀態變了,
    不能只因為「改動有理」就放過。做法是把掃描常數**單獨**改回舊值再跑一次:
    **逐位元回到舊值**。這一次實驗同時證明了兩件事——

    - 硬化護盾 / 掃描器抵銷 / 全艦隊偵測源三項對畫廊輸出**零影響**(沒有意外副作用);
    - 整個差異(含指紋)完全來自掃描範圍那三個常數,路徑是畫廊腳本挑「**第二顆可見的星**」
      當遷移示範目標——可見星變少 → 換了目標 → 集結點是持久化的 → 指紋變。

    可見星變少本身是對的:空間掃描儀 = 2 parsec = 基礎值,手冊叫它「the standard scanner」,
    它就是基準本身。`ParsecToNormalized` 的調參註解寫的目標是「基礎 2 + 母星星基 2 = 4 parsec」,
    **新行為剛好落回那個註解說的設計意圖**,是舊程式碼意外地寬鬆。

    截圖:04_galaxy / 12_planets / 27_commandpoints / 28_measure / 29_confirm / 30_netwait。

143. **盤點:「元件表有」不等於「效果有接」**(2026-08-08)。

    使用者要求盤點剩餘工作。照規矩先查證再回答——結果查證本身變成這一項。

    ### 三個數字都不對

    活表上寫著「④ 剩 6 個元件,各自擋在不同的缺席系統後面」。逐項查完:

    - 真的還擋著的是 **3 個**,不是 6 個;
    - 另外 **3 個的擋門理由已經過期或本來就錯**;
    - 還有 **2 個根本不在那份名單上**。

    ### 漏得最徹底的那一個

    **隱形裝置在 `SpecialOptions` 裡,而整份程式碼沒有任何地方讀它。**
    裝得上、花得了錢、不做任何事。手冊給的規則是完整的:

    > In combat, as long as the ship does not attack, it has an **+80 bonus to its defense
    > against beam weapons** and **missiles and torpedoes have a 50% chance to miss**. When a
    > cloaked ship does attack, it loses these bonuses. Once the ship has attacked, it must
    > remain uncloaked until it spends **one full turn without firing**; then it can recloak.

    最後那一句的語意,就是第 140 項為了超載電容建的 `Fired`。

    **為什麼所有盤點都沒抓到它**:先前每一次盤點問的都是「元件表裡有沒有這一項」。
    隱形裝置**有**,所以每次都被算成「已完成」。
    **「元件表有」與「效果有接」是兩件事,而先前沒有任何一種問法分得出來。**

    新問法很簡單:掃元件名在**元件表以外**的程式碼裡出現幾次,零次就是裝飾品。
    32 個特殊元件跑出兩個——隱形裝置(真漏)與重生程序(已標註的種族特性代理,非漏接)。
    武器 26 項、護盾 6 項全部有消費端。

    ### 三個過期的擋門理由

    | 元件 | 舊理由 | 現況 |
    |---|---|---|
    | 能量吸收器 | 「需儲能狀態」 | 第 140 項建了跨回合的 `Charged`/`Fired`;手冊指名的例外(位移裝置)第 133 項也在 |
    | 戰鬥艙 | 「remake 沒有逐元件佔格的造艦模型」 | **這句話是錯的**——`ShipHullSpace` 與 `ShipDesignSpaceUsed` 都在 |
    | 相位匿蹤 | 「需戰鬥可見性」 | 只需要「不可被選為目標」一個旗標 + 回合數(`t.round` 已有) |

    第二列與第 142 項的掃描範圍是同一種:**不是過期,是當初就寫錯**。

    ### 還有一項從來沒被列進來

    **測距瞄準器**(Rangemaster Targeting Unit)整份程式碼零提及,而第 133 項那次
    「手冊標題對四張元件表」的盤點沒抓到它。手冊規則也是完整的:
    「reducing the absolute range … to one-third of the actual range. Note that the
    dissipation of damage potential is not affected by this system.」
    舊理由「快速結算固定 range=2」只對一半——**格子戰術傳的是真距離**。

    ### 清單自己過期了

    `internal/shell/ship_systems.go` 檔尾那份「這一輪沒有接的,與各自的理由」,
    上一版列 12 項,其中**七項在第 134–141 項已經接掉**(增強引擎 / 時間扭曲加速器 /
    結構分析儀 / 阿基里斯瞄準器 / 超載電容 / 快速飛彈架 / 轟炸機庫),理由卻還留著。
    **那份清單自己就是它警告的那種東西。** 已整份重寫,並分成「真的還擋著」與
    「理由已過期」兩段——後者才是下一輪該做的。

    這一項沒有程式碼改動的功能效果,截圖無差異。

144. **三份文件對齊現況**(2026-08-08)。

    使用者要求更新 `CLAUDE.md`、`CONTEXT.md`,並重新盤點 worklist。

    ### CONTEXT.md 先前不存在

    rulebook/50 要求每個 repo 有一份 domain glossary,而這個 repo 一直沒有。
    收的詞以「**這個專案實際踩過的混淆**」為準,不是通用名詞解釋:

    - **快速結算 / 格子戰術** — 戰鬥的兩條路徑。第 133–142 項有五項的缺陷都是
      「只接了一條」,所以這一組放在第一節,並附上那句該問的話:「**兩條都接了嗎**」。
    - **元件表有 ≠ 效果有接** — 第 143 項的教訓,直接收成一個詞條。
    - **擋門理由** — 附上「會過期、而且更糟的是可能當初就寫錯」。
    - **狀態指紋 / 畫廊 / 棘輪** — 三個驗收工具,各自寫明**它不能證明什麼**
      (棘輪不是覆蓋率;測試綠不是對齊原版)。

    末尾的「Flagged ambiguities」收了四個**還沒收斂、不該擅自收斂**的詞:
    還原度 %(不自評)、戰鬥速度(手冊與 remake 兩種尺度)、掃描器(科技 vs 系統,
    第 142 項之前被混為一談)、戰機型別數(手冊四種、remake 三種)。

    ### WORKLIST 的標題與內容已經對不上很久

    頂端那一段掛著「剩餘工作清冊」,而底下的 A–AO 表**40 列裡 38 列是 ✅**
    ——它早就變成已完成日誌了。任何人打開 WORKLIST 找「還剩什麼」,看到的是一張已完成清單。

    重寫成六節的剩餘工作表(手冊忠實化殘量 / 探針殘量 / 卡在證據不足的 / 還沒排的功能項 /
    刻意不做的留白 / CLAUDE.md 交付項查核),A–AO 表移到下面並改標題成「已完成日誌」,
    附一句說明為什麼留著:**它記的「怎麼被找出來的」比「做了什麼」有用。**

    順帶查了 CLAUDE.md 自己列的交付項有沒有被追丟——`PLAN.md`、致謝、文化現象、
    中文社群考據、sprite/tile 畫質、UI 調整可行性、主選單語言切換,**七項全部在**。

    ### 舊 Phase 區的 32 個待辦方框

    沒有逐條改,而是在頂端寫明**為什麼不能信**,並舉兩個例子:
    「OCR 掃描版手冊附錄」第 123 項用可抽文字的 patch 1.5 PDF 解掉了、
    「決定性化 + lockstep」第 72–79 項整塊完成。
    逐條追認要花的時間遠超過它的價值,而**「要判斷某項做了沒就 grep 程式碼」這條紀律
    比追認更耐用**——追認完的清單明天又會開始過期。

    無程式碼改動,截圖無差異。

145. **第二次 review:錯誤斷言的副本比原件多**(2026-08-08)。

    使用者要求「再 review 一次確保沒有遺漏」,並補了一句「刪除錯誤的記憶」。

    ### 上一輪處理的是原件,而副本散在別處

    第 144 項只動了 `WORKLIST.md`。這一輪掃過所有 markdown,發現同一批假斷言在
    **會被真正引用的文件裡都有副本**——而讀的人多半只讀副本:

    | 檔案 | 假斷言 | 事實 |
    |---|---|---|
    | `README.md` | 「行星表面 + 建築 sprite 擺放卡在幾何表尚未抽出」 | 7×7 角點表與 `Cache_Load_Bldg_` 算式都抽出來了 |
    | `README.md` | 「戰機/航母未做」「多 AI 目標選擇為索引順序」 | 兩者都完成(第 69/141 項、`AIEnemyColonyValue`) |
    ...
    | `component-values.md` | 「裝甲缺科技倍率」 | 第 132 項已解 |
    | `newgame-flow.md` | 「版面為合成近似,尚未像素對齊」 | 2026-08-07 已用執行檔立即數座標 |
    | 本文 | 「戰機/航母戰鬥子模型」仍缺 | 同上 |

    `oracle-comparison` 那一列最值得記:**同一份文件自己內部就矛盾**——結論段寫對了、
    表格沒跟著改。只掃表格的人會拿到相反的答案。

    ### 長期記憶裡有兩份是錯的

    - `moo2-fidelity-20pct-not-test-green` 寫著三句話,**每一句後來都被推翻**:
      「像素對齊已做到 oracle 上限,colony/races/newgame 只能維持 PIL」(反組譯全都有)、
      「至此無清楚定義的純自驅剩項」(之後找出數十項)、「用 archive.org 當 oracle」
      (使用者明確禁止)。已大幅刪修,只留耐用的方法論。
    - `moo2-oracle-is-archive-org-online` 的**檔名意思與內容相反**——內容說「不要用
      archive.org」,檔名讀起來像「oracle 就是 archive.org」。召回時只看得到檔名與摘要的場合,
      它會把人推向使用者禁止的做法。已更名為 `moo2-verify-with-dosbox-not-archive-org`。

    新增一份 `moo2-stale-blocking-reasons`,收第 111–143 項那五條盤點紀律。

    ### 最貴的一句話

    `CLAUDE.md` 原本列了兩個「已知會被誤引的過期錨點」,現在是三個。新增的那個是
    **「已經沒有清楚定義的自驅剩項了」**。

    它 2026-07-12 同時寫進了文件與長期記憶,而之後第 111–143 項光是手冊忠實化就找出數十項。
    **「沒有剩下的了」比任何一個具體的錯誤都貴,因為它讓下一輪不去找。**
...
148. **改政策:過期的斷言直接刪掉,不留刪節線**(2026-08-08)。

    使用者:「刪除錯誤結論(從 markdown 裡面移除,避免混亂),worklist 也重新檢視、移除錯誤的紀錄。」

    ### 先前的做法製造了新的雜訊

    第 144–146 項用的是「`~~錯誤斷言~~` + ⚠ 訂正註記」。當時的理由是保留時序。
    實際效果是**同一句話在正文裡出現兩次**——錯的那次還排在前面。
    掃過去看到的第一眼仍然是錯的內容,而刪節線在很多渲染器裡不明顯。

    改成:**錯誤陳述整句刪除,只留正確結論。** 全 repo 清掉 42 條追認註記、
    十幾處 `~~...~~` 引述,含 `audio-track-map.md` §7.5 那一整節錯誤推導。

    ### 但保留兩類東西

    - **「這件事做完了」的完成標記**(`~~題目~~ **已完成**`)——那不是錯誤結論,
      刪了會毀掉「做過什麼」的紀錄。
    - **錯誤是怎麼發生的教訓**,改寫成**正面敘述**。例如 §7.5 現在只留:
      「零命中時先做正對照;掃 `.asm` 文字而不是查 IDA 的 xref 圖是放大器。」
      ——不必為了講這個教訓而把錯誤結論再抄一遍。

    這一項本身就是分界:**`docs/re/01-gap-report.md`(本文)仍然保留錯誤推導的全文**,
    因為它是工程日誌,「當初怎麼推錯的」正是它的內容;
    **其餘所有文件只留現況**。要查某項當初怎麼錯,來這裡。

    ### 順帶修掉一個會重蹈覆轍的坑

    `audio-track-map.md` 的待辦裡還有 `obj1+0x9082` 這種**除錯表偏移**。
    那正是第 147 項踩到的坑(少加 `0x10000` 的 object base)。
    已在該行標明要用 IDA 資料庫的線性位址,並附上換算。
