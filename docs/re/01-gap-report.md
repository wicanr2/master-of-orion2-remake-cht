...
31. **原版 PRNG 照抄 + 建築擺放換成原版演算法 + 地表底圖接上**(2026-08-07,第 30 項的續)。
    ...
    - **軌道衛星沒畫**:分類 7 的建築(星基 / 次元傳送門 / 天網 / 星際要塞 / 戰鬥站)進的是
      `word_19F99C` 那份清單,由 `Draw_Colony_Satellites_` 另外畫在軌道上。remake 目前
      **完全沒畫**——這顆星有星基與次元傳送門,畫面上看不到任何東西。
      幾何在 `sub_BDFB9`(x 以 `±i×50 + 295` 排開、y 基準 162),暫存器別名很重,還沒解乾淨。
    - **擺放的最後一段微調**沒做(見上表)。影響只在房屋彼此的相對位置。
    - **道路** `Draw_Road_List` 沒畫。
