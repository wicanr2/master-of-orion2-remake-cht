...
| **星圖:遷移連線**(4 層裡剩的那 1 層)| 資料模型 | 蟲洞(35)、星雲(44)、星門(47)、外交燈號(50)已打通;**遷移連線卡在「艦隊是單一集合」這個模型**——要畫「從哪個殖民地生產、送到哪」得先有逐殖民地的造艦佇列 |
| **建築 48 人造行星** | 資料模型 | 真值已抽出(第 36 項),效果在手冊裡(第 51 項訂正:先前的「零命中」是連字造成的假陰性)。**卡在一星一行星模型**:人造行星按定義是在既有星系裡再多一顆世界,remake 的 Stars↔Planets 是一對一 |
| **鍵盤快捷鍵** | 未實作 | 第 54 項副產品:F1/F2 循環艦隊、F5/F6 切換已殖民星系、F9 測距、F10 快速存檔、ALT+F9 載入。**F9 尤其該做**——秒差距模型(第 45 項)已經建好,但玩家在畫面上看不到任何秒差距數字 |
