...
## 這 150 項煉出來的規則(先讀這一段,底下 150 項是參考資料)

> 150 個編號項裡的教訓高度重複——「擋門理由會過期」出現十次、「兩條戰鬥路徑」
> 出現在第 133–142 項每一項。合併成下面 12 條;**要用的是這些,不是逐項讀完**。
> 每條後面是產生它的項次,要看證據再去翻。
...
1. **擋門理由會過期,而且可能當初就寫錯。** 「為什麼這項還沒做」那句話在寫下的那天
   ...
   〔117 118 122 123 130 131 133 139 142 143〕
2. **「元件表裡有」不等於「效果有接」。** 一項可以在表裡、花得了錢,而程式碼沒有任何
   地方讀它。盤點時要分開問這兩件事;偵測法是掃名字在表以外出現幾次,零次就是裝飾品。〔143〕
3. **任何清單都會過期,包括警告別人清單會過期的那份清單。** 做完一項就回去刪那一行。〔143〕
4. **「已經沒有剩下的了」是最貴的斷言**,因為它讓下一輪不去找。〔111–150 全部〕
...
7. **一個元件常有兩個效果,只接一半不會有任何徵兆。** 從單一檔案回推元件效果會漏東西;
   要從**手冊那一段的每一句**回推。〔133 134 142〕
8. **資料層跟著實作層缺,是很難發現的洞。** 單看資料層會以為「這一組本來就只有這些」。〔141〕
9. **測試名稱裡的「只」是一種斷言**,而它斷的往往是你當時的理解,不是手冊。〔142〕

### 三、關於逆向

10. **一手來源永遠贏二手推論。** 一手 = 執行檔立即數、LBX 位元組、手冊行文;
    二手 = 檔名、美術字樣、命名習慣。**openorion2 沒有 ≠ 沒有真值**——反組譯全都有。〔41 129 132〕
11. **不要 grep `.asm`,要查 IDA 資料庫的 xref 圖。** `.asm` 是攤平的文字,沒有交叉參考。
    而且除錯符號表的 `obj1+偏移` **不是**線性位址(差一個 object base)。〔147〕
...
- **狀態指紋變了就代表持久化狀態變了**,必須查出原因,不可因為「改動有理」就放過。〔142〕
...
148. **文件政策:過期的斷言直接刪除**(2026-08-08)。
    ...
    **全 repo 一條規則,沒有例外:任何文件都不留錯誤斷言的內容。**
    ...
    - **教訓,一律正面敘述**。例如 §7.5 只留:「零命中時先做正對照;
      掃 `.asm` 文字而不是查 IDA 的 xref 圖是放大器。」
      ——講得清楚,不必把錯誤原文抄一遍。

    ### 順帶修掉一個會重蹈覆轍的坑

    `audio-track-map.md` 的待辦裡還有 `obj1+0x9082` 這種**除錯表偏移**,
    正是第 147 項踩到的(少加 `0x10000` 的 object base)。
    已標明要用 IDA 資料庫的線性位址並附換算。

    無程式碼改動,截圖無差異。

149. **本文的職責:RE 硬資料 + 工程日誌,不是現況**(2026-08-08)。

    **剩餘工作表在 [`WORKLIST.md`](../../WORKLIST.md) 頂端,那裡是唯一活來源。**
    `CLAUDE.md` / `CONTEXT.md` / `HANDOFF.md` / 五份 kickoff 的指標全部指向那裡。

    本文開頭原有的那份活表已刪除——**兩份活表並存,兩份都會過期**。
    ...
    **要知道「還剩什麼」看 WORKLIST;要知道「某個數字從哪來」看這裡。**

    無程式碼改動,截圖無差異。

150. **本文的錯誤紀錄清除**(2026-08-08)。

    依第 148 項的規則,把本文裡 16 處**複述錯誤斷言**的段落刪掉,只留正確結論:

    | 改成 |
    |---|
    | 殖民地畫面的版面與框架美術**都有真值來源**,而且兩者都不在 COLONY.LBX 裡 |
    | 飛彈基地/地面砲台的空間預算模型**早就在**,第 37 項把它當成缺口是漏查 |
    | `Cycle_Ship_Icons_` 不是動畫,而艦隊圖示在原版也不會動 |
    | 安塔蘭母星防禦艦隊的數字**執行檔裡有**,而且組成不是同級的 |
    | 手冊三個掃描科技的條目**各自明寫著自己的 parsec 值** |
    | 元件表的「舊理由」欄 → **「為什麼現在做得了」** |

    保留的判準見第 148 項。標題裡的「X 不是 Y」句式(如「index >= 0xF0 從來不是顏色」)
    陳述的是**正確事實**,只是用對比句,不算錯誤紀錄。

    無程式碼改動,截圖無差異。

151. **150 項煉成 12 條規則**(2026-08-08)。

    使用者:「合併規則,150 條太多了。」

    那 150 個編號項裡的教訓**高度重複**——「擋門理由會過期」出現十次、
    「兩條戰鬥路徑都要接」出現在第 133–142 項每一項、「一手贏二手」散在十幾項裡。
    逐項讀完才拿得到規則,那個成本沒有人會付。

    合併成 **12 條**放在本文最前面,分四組:
    ...
    每條後面標出**產生它的項次**,要看證據再去翻。
    **150 項就此降級成參考資料**;`CLAUDE.md` 與 `CONTEXT.md` 的指標同步改成
    「只有那 12 條該讀完」。

    順帶清掉四處「留著是為了看得到當時的判斷」的超期內容
    (本文的殖民地畫面中段那節、WORKLIST 的生物武器擋門、HONEST-STATUS 的行星表面、
    `colony-economy-maintenance.md` §2.2 修復前的實測軌跡)。
