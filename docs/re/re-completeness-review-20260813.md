...
- 原版 AI、事件、間諜、艦隊航行、研究完成、安塔蘭入侵等仍有未閉合子系統；殖民地完整回合的
  一次套用／兩遍重算外層已於 2026-08-28 閉合，子公式仍依矩陣分列追查。
...
原版 `Resolve_Spies_ @ 0x10192B` 是回合主鏈直接子系統。Go 已接 STEAL／SABOTAGE／HIDE、Agent、
部分 raw helper 與門檻，但訓練 30 BC、每回合維護 1 BC、AI 任務政策、AB／DB 結構化分數仍明示
為 remake 模型。判定：玩家 UI 與效果可玩，完整原版間諜回合解算未閉合。

### 艦隊航行

原版 `Move_All_Ships_Toward_Stars_ @ 0xFFEEA` 每回合推進艦艇位置。Go 以整段 ETA 表示航行，途中
沒有座標，並明示直線／拆艦限制與原版資料模型不同。判定：基本派遣可玩，但中途截擊、合併、
封鎖、星雲／黑洞／干擾器與抵達時序尚未形成原版垂直鏈。
