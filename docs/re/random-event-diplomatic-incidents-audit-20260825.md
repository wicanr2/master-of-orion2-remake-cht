...
舊版第 3／4 點把 `0x4E972` 以後的夾值視為事件可達路徑，這是錯誤的控制流
斷言。新的 IDA Pro 9.4 完整函式匯出證實：

- 事件呼叫把 record `+0x03` 置於 `EBX`，record `+0x00` 置於 `EDX`。
- `Change_Relations_` 以 `EBX` 帝國記錄為基底讀取 `[EBX player + EDX + 0x627]`，
  即第二帝國對受害帝國的反向正式狀態。
- `0x4E75C..0x4E764` 在該反向狀態 `>=4` 時直接跳到 `locret_4E9FD`。
  關係分數寫回 `0x4E7FA..0x4E852` 與後續夾值在此路徑不可達。
- 因此，若兩方戰爭狀態對稱，事件 4／5 會產生新聞，但不改寫關係分數。
  只有「受害方視為戰爭、第二方反向狀態仍小於 4」的不對稱原版狀態，
  才能繼續到分數公式。remake 現有條約模型是對稱的，不應捏造這種邊。

勘誤證據：`docs/re/evidence/change-relations-ida-20260827.json`；輸入與本頁相同，目標
`sub_4E3B5 @ 0x4E3B5..0x4EA03`，函式 bytes SHA-256
`e0e2c21c0040c22b2d80b6d6947e0c5892963c531985ebb3549f35adaaec8978`。
...
2. 傳入 `EAX` 的原始基礎關係變化：
   - 事件 4：`0xFFFFFF9C = -100`；
   - 事件 5：`0x00000064 = +100`。
3. `Change_Relations_` 確實含有依目前 signed-byte 關係值、actor 政體與目標魅力
   旗標調整增量的共用公式；但在對稱戰爭狀態下，事件 4／5 會在分數
   寫回前直接返回。這些公式只能支撐其他可達 caller，不得再冒稱為對稱戰爭
   事件效果。
4. `0x4E987..0x4E9A3` 的 `-25` 夾值區塊保留在原函式中，但對本事件的對稱戰爭
   路徑不可達；舊版「聯姻改善分數後夾到 -25」已撤回。
