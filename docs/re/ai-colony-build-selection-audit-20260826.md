...
remake 已以獨立可存檔欄位保存目前 Capitol 行星、欄位存在狀態與失守後待重建狀態，
不再用 `Colonies[0]` 冒充；攻陷、重指派、士氣與 raw 9 完工鏈見
`docs/re/capitol-state-audit-20260826.md`。

### 證據等級與剩餘限制

- raw 1／11／14／18／48 固定零分：**已證實**。
- `player+0x29` 為目前 Capitol 行星索引、raw 9 條件與分數：**已證實**。
- 首都攻陷後的人口最高重指派、士氣懲罰與存檔欄位：**已證實並已實作**；這是 raw 9
