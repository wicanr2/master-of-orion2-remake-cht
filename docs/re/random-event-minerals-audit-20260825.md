...
2. `sub_23DA0 @ 0x23DA0` 以 reservoir sampling 均勻抽取目標玩家殖民地，並要求
   `colony+6 == 0` 及 `colony+0x13F == 0`。`+0x13F` 已由 Capitol 狀態鏈證實為 raw 9
   建築槽，因此此條件會排除已完成 Capitol 的殖民地。
...
- 已依上述證據實作礦產條件、200 次拒絕取樣、精確增減量、上限、殖民地產能回寫，以及
  玩家／熱座／AI 路徑。
- remake 已以 `ColonyBuildings` 的 Capitol 對等狀態接入事件 11 的 `sub_23DA0` 候選；
  事件 12 維持使用不排除 Capitol 的 `sub_23D44`，兩者不再共用錯誤的泛用抽樣器。
