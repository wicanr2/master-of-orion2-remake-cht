...
全表使用手冊 p.124-125 真值，逐項出處見 `internal/gamedata/weapon_damage.go`：
...
裝甲／護盾另計：護盾使用手冊減傷值；裝甲基準值依艦體計算，科技倍率另由手冊
Ship 條目提供。

裝甲科技倍率依手冊為 +100%／+300%／+500%／+700%／10 倍，實作位於
`gamedata.ArmorStructurePercent`。1.31／1.5 規則值由 `internal/gamedata/ruleprofile.go`
的 `RuleProfile` 分版；目前待辦只看 `WORKLIST.md`。
