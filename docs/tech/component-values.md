...
⚠ **2026-08-08(第 65 項)作廢並訂正。** 那個單調估計錯得比想像中遠,而且**排序被弄反**:
...
裝甲/護盾另計:護盾已於第 63 項改用手冊值;裝甲的手冊基準值依**艦體**而非元件,
而裝甲科技的倍率手冊沒給、openorion2 也沒有,**仍缺**(見第 64 項)。

## 待辦(精確對齊的可行路徑)

- [x] ~~OCR 掃描版手冊武器附錄(9 頁本可能不含,需找完整手冊)~~
      **⚠ 這條的前提是錯的**:`moo2_patch1.5/GAME_MANUAL.pdf` 一直都可以直接抽文字(44 萬字元),
      武器表就在 p.124-127。同一頁的 Size 欄第 17 項就抽過了(`WeaponSpaceByName`),
      只是當時沒順手抽旁邊的 Damage 欄。**不需要 OCR,也不需要另找手冊。**
- [x] **裝甲科技倍率第 68 項已解**——手冊 Ship 條目逐級寫著
...
- [x] ~~或逆向私有 gamedata 武器資料表,寫 dumper 抽精確值~~
      **不需要**:第 65/65 項已從可抽文字的 patch 1.5 PDF 拿到全表(18 把武器)。
