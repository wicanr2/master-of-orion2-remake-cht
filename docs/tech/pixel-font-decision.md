...
| 驗證項 | 結果 |
|---|---|
| 字元覆蓋(抽樣) | 8 行**合計缺字 0**,無 tofu(含 隆/瑞/薩/崔/矽 等密筆劃繁體字) |
| 字元覆蓋(**全語料窮舉**) | 全 24 個 i18n TSV + 全 .go 硬編中文 + 星名/艦名池,共 **2258 個唯一 CJK/全形字元,缺字 0**(對齊 `08` §4「窮舉全文字集」)。→ Noto fallback 非覆蓋所需,仍保留當防禦(未來新增字) |
...
## 4. 設計後果 → 使用者回饋修正為「混合字型」

**初版(Stage 1)**把字級收斂為 2 級純點陣:內文 12px 1×、標題 24px 2×(最近鄰放大)。
但使用者實測 Stage 1 前後對照後給出逐畫面回饋,推翻「標題也點陣」:

| 使用者回饋 | 修正(Stage 2) |
|---|---|
| 戰術標題 2× 點陣「有點鋸齒」 | 大字改走**平滑向量**,不用點陣放大 |
| 主選單「維持改前(Noto)」 | 主選單整個用**純 Noto 向量** |
| 殖民地內文點陣「OK」 | 內文維持點陣 |

**最終設計 = 混合字型(hybrid)**,`internal/uifont` 以 `hybridThreshold=18` 分流:

| 字級 | 走向 | 用途 |
|---|---|---|
| **< 18px** | bitmapfont FaceTC 點陣 | 內文/表格/按鈕/艦名(銳利、貼原版美術) |
| **≥ 18px** | Noto 向量(平滑) | 標題(避免點陣 2× 放大鋸齒) |

**主選單例外**:整個畫面用**純 Noto 向量**(`sceneBuilder.fntVec`,不套點陣)——使用者偏好
LCD 面板上的平滑觀感。`-lang en` 全程本就純 Noto,不受影響。

**點陣 12px 下限的副作用**:size<12 的小字被撐大到 12px,密欄清單易溢出 cell
(如殖民地「已建:…」建築清單撞出「建造」欄)→ 以 `truncateToWidth`(依欄寬截斷+省略號)
處理,不縮字(點陣無法再縮)。視覺層級由「點陣/向量 + 大小 + 顏色 + 位置」共同建立。
