...
> **本節已過時,完整版見 `docs/tech/weapon-mods.md`。** 當時只查到 p.128(Design Dock 說明頁)
> 對火線角(Firing Arc)與部分 mod 的重申,精確數字其實在 p.115-118「Modifications」章節
> (`GAME_MANUAL.pdf`,「Weapons Upgrades」小節)才是完整、逐項給數字的原始出處——CO/ENV/NR/SP
> 這幾個 mod 當時漏收。武器改造(mod)系統已在本輪(`internal/gamedata/weapon_mods.go` +
> `internal/shell/weapon_mods.go` + `combat_formula.go` 的 `ResolveShotWithMods` +
> `cmd/moo2/interactive.go` 艦艇設計畫面)接線完成:佔格(`ShipDesignSpaceUsedWithMods`/
> `ShipDesignFitsWithMods`)、成本(`DesignCostWithMods`)、命中/傷害(`ResolveShotWithMods`)
> 皆已生效,不再是「已確認,未接線」。留下此節只為保留歷史紀錄與 p.128 出處交叉引用,**佔格/傷害
...
艦艇設計的工作狀態只在 `WORKLIST.md` 維護；原版對齊邊界見
`docs/re/parity-matrix.tsv`。本文件只保存空間模型與測試證據，不複製待辦。
