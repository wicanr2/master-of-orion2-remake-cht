...
| `income.go` | 稅率/稅收、貿易財、糧食剩餘、指揮溢出、運輸艦維護、政府金錢加成 | GAME_MANUAL.pdf p37/70/169 | `income_test.go` |
| `morale.go` | 每格士氣 ±10%、政府基礎士氣、首都淪陷、多種族、建築、Unification 特例 | GAME_MANUAL.pdf p65/165-170 | `morale_test.go` |
| `ground.go` | Barracks 上限、單位血量、裝甲戰力加成、種族修正、轟炸 | GAME_MANUAL.pdf p24-129 | `ground_test.go` |
| `damage.go` | 距離衰減表、DamageForHit、護盾、裝甲穿透、球形武器 | 手冊 p120-128 | `damage_test.go`(含 Laser 誤差標註) |
| `internal/engine/` | 回合引擎:殖民經濟/研究/**BC 結算**/帝國編排 + save↔engine adapter | 組合上述公式 | `*_test.go`(含多回合推進、真實存檔轉換) |
