...
譯文以**英文原文為 key**存在 `assets/i18n/*.json`,渲染時 `Catalog.Translate(englishText)` 查表換中文。
...
我們的顯示層覆蓋改用「英文文字」當 key,若把**所有 JSON 合併成單一 catalog**,同形詞會碰撞(後載入者靜默覆蓋)。

## 架構決策:per-source catalog

**忠於 MOO2 的 per-table 設計:每個字串來源(JSON)載入為獨立 catalog,渲染某來源的字串時用該來源的 catalog 查。**
如此同形詞各表獨立、各自正確,不需強制全域統一。`cmd/moo2` 目前各 demo 模式即各載一個 TSV,符合此模型;
未來完整遊戲迴圈應維護 `map[source]*Catalog`,而非單一大表。

## 同形詞稽核(2026-07-03)

跨 JSON 來源同英文 key 但譯文不同者共 32 組。處理原則:

- **直接可比較的畫面 → 強制統一**:`races.json`(種族選擇)與 `raceinfo.json`(種族資訊面板)是同一批 RACESTUF 特性,
