...
零緩衝經濟 → 隨機事件扣人口 → 食物赤字 → 原版 signed 成長按短缺形成負值 → 人口減員。

現行 `recoverFromFamine` 會在饑荒時每回合把一名工人／科學家改派農夫；逐族食物／工業消耗、
負成長池與刪人口則已依 `sub_DEB4B`／`sub_DF546`／`sub_E1839`／`sub_E2DCA` 接線。早期文件所稱
「沒有自動改派、饑荒只停止成長」已被程式與新 RE 證據推翻。
...
「當時尚未接自動改派與原版負成長」疊加後的**歷史性結構死結**。任務指示的判斷準則
...
**`TradeGoodsIncome` 當時刻意仍未接**(§2.3 選項①的另一半,本輪判斷):手冊定義的貿易財收入來自
玩家把部分工人產能「明確配置」去生產 Trade Goods,當時誤判為需要「第四種職務分配」
(非 Farmer/Worker/Scientist 三者之一)才能接。

> ⚠ **2026-07-11 更正**:上述判斷是誤判。貿易品實際是「建造佇列選項」(與 Housing 同類——
> `ColonyState.Housing` 早已是先例),不是產能分配職務——殖民地把建造設為「貿易品」時,
> 其 `NetIndustry`(已由三種職務算好)整包不蓋建築、改以 2:1 換 BC,不需要新增第四種職務
...
- AI 母星 yield 仍是 placeholder(§6.3):待 `advanceAI` 的整數捨去 bug(§3)另案修好後,
  ...
  兩個函式可以考慮合併回一個。
- 運輸艦、指揮點超支、間諜與軍官維護費均已接入同一次帝國經濟結算；間諜每名 1 BC 與
  軍官基本式已有 IDA Pro 證據。原始證據見 `docs/re/player-maintenance-audit-20260825.md`。
