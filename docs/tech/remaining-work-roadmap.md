...
| 項目 | 說明 | 解鎖後 |
|---|---|---|
| **開局經濟平衡** | 本 session 數處改了開局狀態:士氣 +10→**0**(手冊忠實)、指揮點數 **-20 BC/回合**(手冊忠實)、領袖研究 **+25**(正 bonus 抵銷)。三者單獨都忠實,複合後 demo 開局偏 BC-負,但「原版開局是否真流血」無 turn-1 oracle 無法定論 | 你回報「正常/太苦」→ 我接 income 死碼(政府 money bonus、morale 對收入的影響、貨運維護 `income.go`,均已移植);太苦則我查指揮供給模型是否缺源 |
