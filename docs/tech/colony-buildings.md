...
| 次元傳送門(Dimensional Portal,p.106,Satellite) | **2026-07-11 第二輪已接線**:先前只記錄「已建」(顯示用),「解鎖反攻安塔蘭母星」這個效果是死資訊。現由 `internal/shell/antaran_victory.go` 的 `hasDimensionalPortal()` 讀取 `s.ColonyBuildings` 判定是否已建成(任一殖民地即可,remake 簡化,見該檔案註解),`CanAssaultAntares()`/`AssaultAntares()` 依此決定是否允許發起手冊第二條勝利路徑(反攻安塔蘭母星)。維護費 2 BC/回合本來就已計入既有 `totalBuildingMaintenance`(建築通用機制,非本輪新增)。詳見 `docs/tech/victory-conditions.md` §4.4。 | `ColonyBuildings["次元傳送門"]`(供 `hasDimensionalPortal` 讀取) |
| 地形改造 / 蓋亞轉化 / 土壤改良(Terraforming/Gaia Transformation/Soil Enrichment,p.99-101,「Special」型別,**不計入本表 40 項**) | **2026-07-11 已接線**:`internal/gamedata/terraform.go` 移植好的公式先前零呼叫端。新增 `engine.ColonyState.Climate` 欄位(零值陷阱處理同 `PlanetGravity`/`MineralRichness`,`gamedata.TOXIC` ordinal=0;此欄位不像 Gravity/MineralRichness 被每回合核心公式讀取,只在地形改造/蓋亞轉化套用瞬間讀寫)。新增 `internal/gamedata/special_actions.go` 的 `SpecialAction`/`SpecialActions` 表,把這三項排進 `availableBuildOptions`(前置科技:地形改造 `TOPIC_GENETIC_MUTATIONS`、蓋亞轉化 `TOPIC_TRANS_GENETICS`、土壤改良 `TOPIC_ADVANCED_BIOLOGY`,取自 `openorion2/src/tech.cpp` 的 `research_choices[]` 陣列索引=`ResearchTopic` 列舉值,與既有 34 項建築前置科技逐一交叉核對 100% 相符)。`shell.advanceBuilds` 完工時呼叫 `applySpecialAction`——**刻意不記入 `ColonyBuildings` dedup map**(手冊明講地形改造可重複套用,記入 dedup 會被既有「已建過不再套用」邏輯擋下第二次)——依 `TerraformNextClimateOptions`/`GaiaTransformationCanApply` 推進氣候,`ClimateFoodPerFarmer` 差值疊加 FoodPerFarmer(保留既有建築加成不被覆蓋)、`gamedata.TerraformPopMaxAfterClimateChange` 依 pop_climate 係數等比例縮放 PopMax。誠實近似/TODO:①PopMax 縮放非精確重算(remake 無「行星尺寸→基礎人口容量」對映表,對目前整體 PopMax 直接套比例,含已疊加的固定加成如生態圈+2 也會一併縮放,詳見該函式註解);②建造成本(PP)手冊無數據,比照其餘估計建築的 RP 量級外推(地形改造 260/蓋亞轉化 900/土壤改良 150),手冊「地形改造每次套用成本遞增」未模擬(固定成本,不遞增);③Barren 地形改造下一級的兩個候選(Desert/Tundra)手冊未給選擇條件,固定選第一個候選;④土壤改良在 Barren/Radiated/Toxic 星球套用「做了但沒有效果」(誠實模擬手冊「化學反應抵銷肥沃化」,不是在建造選單擋下這個選項本身)。 | `Climate`+`FoodPerFarmer`+`PopMax`(經 `gamedata.TerraformNextClimateOptions`/`GaiaTransformationCanApply`/`TerraformPopMaxAfterClimateChange` 算出) |
...
| 軍事/防禦類(太空學院、飛彈基地、地面砲台、阿提米絲系統網、行星輻射/通量/屏障護盾、曲速力場干擾器、食物複製機、自動實驗室、再生反應爐) | 對應的艦隊駐防/軌道防禦/工業-食物轉換等系統本身尚未建;等對應系統就緒後再回頭補建模。海軍陸戰隊營/裝甲營房的駐軍生成另見 `ground_invasion.go`(海軍陸戰隊營已建模,裝甲營房尚未)。星基/戰鬥站/星辰要塞的**指揮評等供給**已於上表(6.1)接線,但這三項的掃描範圍/艦隊光束攻擊加成敘述效果仍在本行 TODO 之列(掃描/戰鬥加成系統本身尚未建)。 |
