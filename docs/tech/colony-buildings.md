...
| 建築 | 為何不建模 |
|---|---|
| 行星重力產生器(p.104) | **重力懲罰系統本身尚未接進生產管線**——`gamedata.GravityPenaltyPercent`/`GravityAdjustedProduction` 已依手冊移植且有單元測試,但通盤搜尋確認 `internal/engine`、`internal/shell` **零呼叫端**在套用它們(`ColonyState` 也沒有 `PlanetGravity`/種族重力天賦欄位)。這比單一建築固定加成缺口更大,`NormalizeGravity` 旗標先建起來但目前是 no-op,待重力系統本身建好後再接。 |
| 機器人工廠(p.82) | 效果依「礦產豐度分級」給固定值(Ultra Poor~Ultra Rich),但 `ColonyState` 不追蹤逐殖民地的礦產豐度分類(豐度只在建立殖民地當下烘進 `IndustryPerWorker` 靜態費率,事後拿不回原始分類),無法忠實換算,暫不套用任何固定值。 |
| 士氣類(全息模擬艙、歡樂穹頂) | 士氣百分比系統本身有(`MoralePercent`),但這兩棟建築要「調整」士氣值本身(而非套用士氣),`ColonyState` 目前沒有「士氣來源加總」的可寫入介面。 |
| 軍事/防禦類(陸戰隊營房、裝甲營房、太空學院、飛彈基地、地面砲台、戰鬥站、星辰要塞、阿提米絲系統網、次元傳送門、行星輻射/通量/屏障護盾、曲速力場干擾器、異族管理中心、食物複製機、自動實驗室、再生反應爐) | 對應的陸戰隊生成/艦隊駐防/軌道防禦/指揮評等/同化速率/工業-食物轉換等系統本身尚未建;等對應系統就緒後再回頭補建模。 |
