...
| 建築 | 為何不建模 |
|---|---|
| 異族管理中心(p.92-93) | **部分接線但狀態模型不忠實**：同化速率與 `-20%` 士氣消除均有玩家可見消費端；然而 remake 以 `UnassimilatedPop>0` 代理多種族。IDA 已證實原版 `sub_DDAD4` 比較不同正常 race slot，與是否完成同化無關；待 RE gate 後以逐 race population group 修正。見 `docs/re/colony-morale-audit-20260828.md`。 |
