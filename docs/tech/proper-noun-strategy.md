...
| 艦名(672) | **暫緩,但優先順序高於隨機星名** | 樣本(Sparrow I、Starling、Hawk I、Falcon II…)顯示這池不是隨機音節,而是「真實英文詞(鳥類/軍武詞彙)+ 羅馬數字流水號」的固定表,去重後基底詞彙量遠小於 672,且多數鳥類/軍武詞在中文已有慣用譯名(麻雀、獵鷹、老鷹…),規則化翻譯的可行性與品質都優於隨機星名。列為下一輪候選,本輪不落地。 |
...
**暫緩理由**:仍需先完整 dump 672 筆做去重統計,才能估出真正的基底詞彙量與工作量;
在有實際清單前不宜先定案譯名。建議下一輪:先跑一次 dump + 去重統計,若基底詞彙量落在
可一次性處理的規模(如 100–200 組),則排入翻譯 worklist,優先於隨機星名池。
...
| 艦名(672) | 定案「暫緩但優先於隨機星名」+ 可行性方案,待 dump 去重後排程 |
