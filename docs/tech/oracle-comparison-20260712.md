...
**→ issue #2 結論**:原版 Continue 無存檔時**本來就不能按**(灰階)。remake 應:①無存檔時 Continue/Load 顯示為停用、不可點;②Load Game 點下去開「存檔選擇畫面」(原版 Load Game Window)。
...
| 項目 | 原版 | remake | 判定 |
|---|---|---|---|
| 版面 | **肖像在左**大圖 + 右側 **13 族 2 欄按鈕** + Custom | 肖像在左、右側 2 欄 × 7 列按鈕 | ✅ 主要版面與原版座標一致 |
| 族數 | 13 族 + Custom | 13 族 + 自訂種族 | ✅ 數量相符 |
| 肖像/描述 | 原版逐族肖像；選取流程更新肖像 | 用原版肖像；另加外部 JSON 雙語摘要面板 | ◐ 肖像素材與位置一致，摘要面板是 remake 轉接介面 |
| 標題 | SELECT RACE(右上) | 英文保留原版烘字；中文在同一右上標題區擦底疊字 | ✅ 位置一致 |

**→ 現況**：2026-08-07 起 remake 已使用 `RACESEL.LBX` 畫面，按原版立即數重排為左側肖像與右側兩欄按鈕。2026-08-27 起繁中標題、族名、形容詞、摘要與取消按鈕均由 `assets/i18n/ui.json` 供應，文字使用安全框；英文保留原版烘字。額外摘要／取消面板是明示的 remake 轉接介面，不冒稱原版 widget。
...
## 新遊戲流程小結
四張都能玩；種族選擇主要版面已對回原版。仍有差異的是命名與旗色合併成一頁、旗幟以色塊替代，以及少數明示的 remake 輔助面板；這些是視覺／流程忠實度缺口，不是可玩性阻塞。
