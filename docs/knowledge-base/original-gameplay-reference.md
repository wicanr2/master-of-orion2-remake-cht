...
**最關鍵發現:研究節奏過快(已修)**。`ResearchPerScientist=30` 曾是唯一沒有手冊/函式支撐的硬編產出率。**2026-07-12 已修**:base 改 `ResearchPerScientistNorm=3`(手冊 p.949)+ 母星分配對齊 SAVE10 的科3(科≥2 不變式)。載重改動依教訓用 moo2sim 探針驗證(和平開局 BC 健康 56-98→BC50 後 6、安塔蘭極限案有界),未盲改堆疊。使用者 2026-07-12 授權「數學用手冊+攻略完成」,SAVE10+手冊即 oracle,不需逐項 playtest。
