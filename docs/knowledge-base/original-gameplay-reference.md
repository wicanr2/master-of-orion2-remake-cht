...
| 每農食(FoodPerFarmer)= **4** | `climateFoodPerFarmer[TERRAN]`=**2** | ⚠ 差 2x——存檔 4 可能含母星/科技加成,remake 是氣候基準值;需釐清加成來源再定 |
...
⚠ DOSBox 未安裝(私有包有 Orion2.exe/ORION95.exe),自動化原版遊玩→存檔不可行;取得varied 存檔需使用者手動或另建 DOSBox 環境。**食物校正在此暫停(BLOCKED-on-oracle),不盲改**。
