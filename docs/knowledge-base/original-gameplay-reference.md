...
**⚠ 仍待原版存檔(DOSBox)才能定論、不當硬基準**:①銀河議會召開/複投間隔(25 vs 30 回合來源衝突)②各銀河大小恆星數精確值 ③100% 污染行星降級。

### 7.0.1 SAVE10.GAM oracle 反推(原版真值,2026-07-11)

`moo2-private-build/gamedata/mastori2/SAVE10.GAM`(原版存檔,5 玩家各 1 母星、皆 pop8=近 turn-1)用 remake 存檔解析器(`internal/save`)dump 出**原版真值**:

| 原版真值(SAVE10) | remake 現況 | 結論 |
|---|---|---|
...
| 每工業(IndustryPerWorker)= **3** | `MineralIndustryPerWorker(ABUNDANT)` | 待確認該函式回傳值是否=3 |
| 每科研(ResearchPerScientist)= **3** | **硬編 30**(playerHomeworldColony,無手冊出處) | ❌ **疑似 10x 過高**——原版每科學家 3 研究,remake 母星 1 科學家×30=30 研究/回合,原版母星(科2-4×3)約 6-12。**研究節奏可能約 3-10x 過快** |
| 職務分配隨種族變:農3-5/工0-2/**科2-4** | 農4/工3/**科1** | ⚠ remake 科學家偏少(科1 vs 原版科2-4) |
...
- `~/moo2/openorion2/src/gamestate.h:856` — `uint8_t food_per_farmer; // in half-units`(**食物每農夫欄位明註半單位**)。
- `~/moo2/openorion2/src/gamestate.h:857` — `uint8_t industry_per_worker;`(**無** half-units 註解 → 整單位)。
- `~/moo2/openorion2/src/gamestate.h:858` — `uint8_t research_per_scientist;`(**無** half-units 註解 → 整單位)。
- 這就同時解釋了謎題另一半:**為什麼工業/研究沒被「×2」**——因為只有食物欄位是半單位。存檔 `IndustryPerWorker=3` 是整單位 = ABUNDANT base 3(手冊 p.56-57),`ResearchPerScientist=3` 是整單位 = 銀河基準 3。三個欄位單位不同,不是食物被加成、別的沒加成。
- `~/moo2/openorion2/src/gamestate.cpp:398` — `food_per_farmer = stream.readUint8();`(讀原始位元組,不做除法;半單位語意是欄位本身的約定,消費端才配對)。
- 佐證:同結構 `gamestate.h:883-895` 的 `food_consumption_*` / `industry_consumption_*` 皆註「in half-units」——**MOO2 整個食物帳本用半單位粒度**。原因(第一性原理):遊戲存在**分數級食物消費/產出**——Meklar 每人「消耗半單位食物」(手冊 line 596 "half a unit of food each per turn")、Natives 農業 +2、Klackon 農夫 +1、Silicoids 不吃食物等——為了用整數存分數,食物一律 ×2 存為半單位整數。工業/研究基準層沒有這種 0.5 粒度消費者,故用整單位。

**逐一裁決原假設:**

| 假設 | 裁決 | 依據 |
|---|---|---|
| ① 母星專屬 +2 食物加成 | ❌ **否** | 手冊 p.932-944:Large Home World 給人口成長、Rich Home World 給**工業**,無任何「母星 +2 食物」條目;無此機制 |
| ② 基礎食物被全域 ×2 乘數 | ⚠ **形式上像、實質不是加成** | 不是 gameplay 乘數,是**儲存編碼**(半單位)。真實食物 = 存檔值 ÷ 2 |
| ③ 開局共通科技/建築提升食物 | ❌ **否** | 開局保證只有 Capitol/Marine Barracks/Star Base 等(§7.2),無任一給農業加成的建築 |
| ④ 存檔 climate 枚舉序 ≠ remake | ❌ **否** | `gamestate.h:144-155` 枚舉 `OCEAN=5, TERRAN=8, GAIA=9` 與 remake `enums.go` **完全一致**;存檔 climate=8 確實是 Terran |
| ⑤ FoodPerFarmer 欄位存「已乘算後的值」 | ✅ **是,這就是答案** | 欄位存的是「已 ×2 的半單位值」,語意是半單位而非整單位 base |

**完整食物每農夫公式(原版真值):**

```
每農夫食物(整單位) = ClimateBaseFood(氣候)            [手冊 p.59 表:Terran 2 / Ocean 2 / Gaia 3 …]
                     + 種族農業天賦(±N,life-bearing 星球下限 1,手冊 p.707-711)
                     + 建築/科技(Soil Enrichment +1、Weather/Gaia +2、Hydroponic +1…)
                     再乘士氣(±10%/笑臉)與重力懲罰(-25%/-50%)

存檔 food_per_farmer 欄位 = 上式結果 × 2(半單位整數編碼)
∴ SAVE10 母星 Terran:base 2 + 開局無天賦/建築 = 2.0 → 欄位存 4  ✓
```

**remake 忠實建模建議(校正方向,交使用者驗收):**
...
來源:openorion2 `src/gamestate.h:856-858, 883-895`、`src/gamestate.cpp:398`、`src/gamestate.h:144-155`(climate enum);手冊 `GAME_MANUAL.pdf` p.59(Base Food per Unit 表)、p.62-63(Food per farmer 定義)、p.596/707-711/932-944(半單位食物消費、農業修正、母星天賦);[StrategyWiki: Feeding your people](https://strategywiki.org/wiki/Master_of_Orion_II:_Battle_at_Antares/Feeding_your_people)、[Fandom: Farming](https://masteroforion.fandom.com/wiki/Farming)(Ocean/Swamp/Terran base = 2)。

### 7.0.3 研究節奏(已修 base,仍有後續)

**最關鍵發現:研究節奏過快**。`ResearchPerScientist=30` 是唯一沒有手冊/函式支撐的硬編產出率(食物用氣候表、工業用礦產表,都手冊錨定;唯獨研究硬編 30)。原版存檔證實每科學家研究=3。**修正方向**:ResearchPerScientist 改用「每科學家基準值(原版≈3)」+ 母星職務分配科學家數對齊原版(科2-4)。⚠ 這是**開局經濟平衡的載重改動**(研究會顯著變慢),依既有教訓須「先 headless 探針驗開局軌跡 + 使用者 playtest」再定,不可盲改堆疊。
