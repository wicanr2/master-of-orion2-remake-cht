...
- AI 常態研究已依 `All_AI_Tech_Select_ → sub_DC288 → sub_FD335` 改為一次 application 級
  ...
  其餘分數區域、帝國配額、支援艦、戰鬥艦 case 2 改裝、軌道基地、多艦隊分派、外交與其餘
  state machine 尚未閉合，不能以這些切片宣稱完整原版 AI。
...
- 戰術戰鬥已完成第一層固定骨架對拍：太空戰場與 129px `COMBAT.LBX#0` 控制甲板邊界、艦艇
  sprite 與控制甲板資訊層級已取代舊的自製標題、訊息條、可見格線與逐艦卡。公開原版截圖只能
  提供 layout-only 證據；原版自由座標、同艦隊狀態、heading、選取效果、縮圖及動畫 tick 尚未由
  DOSBox 同幀固定，因此不能宣稱戰鬥畫面已逐像素相同。
