...
- 殖民地:原版是**格子地形 + 每格產出 + 30+ 種建築 + 污染/食物/貿易真公式**;我的是三個職務數字,尚無格子地形。**(2026-07-11 更新:40 棟建築已入建造清單/前置科技 gating(`colony-buildings.md`),其中 17 棟已依手冊頁碼忠實建模數值效果——per-worker 產出(自動工廠/機器人採礦廠/深層核心礦場/行星超級電腦/銀河網路中心/研究實驗室/氣候控制器/太空大學)與「殖民地整體固定加成」(自動工廠/機器人採礦廠/深層核心礦場/研究實驗室/行星超級電腦/銀河網路中心/水耕農場/地底農場)已分開累加,不再像先前那樣把手冊的固定值揉進 per-worker 湊數(小殖民地過度受益、大殖民地受益不足);另接了收入百分比(太空港+50%/證券交易所+100%,逐殖民地精確套用)、人口上限加成(生態圈+2)、固定成長點(複製中心)。仍缺:重力懲罰系統本身尚未接進生產管線(行星重力產生器只記錄已建,不影響數值)、機器人工廠的礦產豐度分級未追蹤、士氣類(全息模擬艙/歡樂穹頂)與軍事/防禦類(陸戰隊/戰機/軌道防禦等 ~15 棟)仍只記錄已建、不影響數值——這些需要對應系統(士氣/陸戰隊/艦隊駐防/軌道防禦)先建好。詳見 `docs/tech/colony-buildings.md` 建模狀態一節。仍完全沒有「格子地形」這個更底層的殖民地表現方式。)**
