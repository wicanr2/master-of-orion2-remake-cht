...
2026-08-27 本週收尾的 README 儀表板為：remake 功能 76%、原版玩法對齊 49%、發行驗證 33%。
功能比例由 WORKLIST 26 完成、6 進行中、6 待辦計算；對齊比例由矩陣 10 已對齊、19 部分、
...
- 畫面已有大量原版座標證據，但動態文字測量、中文分頁、按鈕置中、hover 及
  英文模式回歸仍是活躍 polish；README 現行 `25_shipdesign.png` 的空間資訊跨越美術分隔框，
  `23_multiplayer.png` 的說明列則位於主面板外。既有幾何測試只證文字框互不重疊，未證其包含於
  所屬美術面板；不再使用「逐畫面像素對齊已全部完成」的總括斷言。
