# 誠實現況評估(「還原度 20%」已過時,見下方 2026-07-12 里程碑)
...
> **⚠ 2026-07-12 里程碑更新——「20%」是接原版美術前的過期快照,已被大幅超越**:使用者親自校正,那個 20% 是 2026-07-04、還沒把原版 LBX 美術接進畫面前的實測感想,早於美術整合 + 十幾輪 gameplay 死碼接線 + 本日經濟校準。**第一版移植現已實質完整**:①原版美術逐畫面 + 全繁中 ②可玩多帝國 4X 迴圈(玩家+3AI,三條勝利路徑) ③**經濟已逐項對齊手冊真值**(見下段) ④音樂/音效(PCM WAV,場景 BGM + 戰鬥音效接線)。經濟端到端驗證:60 回合玩家 BC 健康爬升(55→419)、3 AI 競爭擴張、無 panic。**還原度%仍不自評、待使用者實測定案**,但機制層已從「自製玩具」進到「手冊錨定」。
>
> **2026-07-12 經濟校準批次(手冊逐字考據 + SAVE10.GAM oracle + moo2sim 探針)**:母星分配農4/工1/科3(SAVE10 不變式)、每科研 30→3、開局 BC 100→50、種族起始金移除(改人頭收入)、開局領袖池清空(原版須雇用)、**收入模型核心校正——補人頭基礎收入每人1BC**(治好低工業母星流血,開局轉 +5 BC/回合,精準重現手冊實況)、稅率預設 15。詳見 `docs/knowledge-base/original-gameplay-reference.md` §7 與 `docs/knowledge-base/manual-cht/`(繁中機制大全)。

**【歷史】2026-07-04 使用者實測時:「能載入原版美術、逐畫面中文化的畫面導覽器 + 幾個自製的玩具系統」,還原度約 20%。**(此頭條為接美術前快照,已被上方 2026-07-12 里程碑超越。)

> **(2026-07-11 重大更新——頭條「不是能玩的遊戲」已過時,依 rule 63 更正)**:經本日 19 輪「接死碼 + 修 regression + 拓殖/多 AI」後,**遊戲現在是可玩的多帝國 4X 迴圈**(以下皆有 headless 模擬探針驗證,非自評):①經濟忠實+平衡(重力/士氣/礦產/建築效果/地形改造/污染/成長/饑荒/維護/收入;開局曾因指揮超支死亡螺旋,已用 SAVE10.GAM 真存檔 oracle 反推補基礎供給=5 修好,開局 BC 健康爬升)②玩家可拓殖(殖民船建新殖民地)③3 個 AI 對手各按性格獨立擴張成長(好戰型戰力遠高於科學型)④可贏一局(征服 + 真銀河議會選舉 + 安塔蘭母星反攻,**手冊三條勝利路徑全接**,玩家+3AI=4 帝國達手冊 ≥3 門檻)⑤戰鬥(beam/飛彈/球狀分流 + 地面戰陸戰隊/戰車/軌道轟炸)⑥指揮點數/領袖技能/間諜偷科技/勝利條件。**這是可驗證的「什麼現在能運作」,不是自評 fidelity%——vs 原版的還原度仍待使用者實測。** 仍缺(增強/打磨非可玩性阻塞):戰機/航母、完整 spy/leader/diplomacy/多AI目標選擇 UI、逐畫面像素對齊、地面戰 d100 結構與飛彈速度的實機校驗、安塔蘭母星防禦艦隊戰力(手冊/openorion2 均無精確數字,現用保守預設,見 `docs/tech/victory-conditions.md` §4.4)。詳見 `docs/tech/remaining-work-roadmap.md`。(註:MOO2 殖民地是人口職務模型非格子地形,先前「殖民地格子」寫法已於 §B 更正。)

> **2026-07-11 第二輪追加**:安塔蘭母星反攻(手冊第二條勝利路徑)已接線——次元傳送門建成後解鎖
> `GameSession.AssaultAntares()`,戰勝即攻陷母星、下個回合判定安塔蘭勝利。防禦艦隊戰力手冊/
> openorion2 均無精確數字,用保守預設(6 艘末日之星等級戰力),已誠實標注,詳見
> `docs/tech/victory-conditions.md` §4.4。

> **2026-07-11 第三輪追加**:武器改造(mod)系統已接線——手冊(`GAME_MANUAL.pdf` p.115-118)
> 8 個光束/通用 mod(HV/PD/AF/CO/AP/ENV/NR/SP)全部逐字核對出佔格/成本/命中/傷害數字,接進
> `ShipDesignSpaceUsedWithMods`/`ShipDesignFitsWithMods`/`DesignCostWithMods`(佔格/超格擋下/成本)、
> `ResolveShotWithMods`(命中/傷害,`battleVolley` 快速結算與 `fireRound` 格鬥畫面共用)、艦艇設計
> 畫面新增 8 個 mod 勾選 chip。無 mod 武器行為逐位元回歸不變(含「無武裝」0 傷害邊界),20 回合
> 開局 BC 走勢(100→130)與既有基準一致,無 regression。**未接**:NR 的實際傷害效果(現行戰鬥模型
> 本身沒有射程傷害衰減可消除,見 `docs/tech/weapon-mods.md`)、飛彈/魚雷專屬 mod(ARM/ECCM/EMG/
...
- 外交/間諜/議會、隨機事件、歐瑞恩守護者、勝利條件——**大多缺席或極度簡化**(安塔蘭母星已接,見下)。
  ...
  **(2026-07-11 第二輪更新:安塔蘭母星反攻(手冊第三條路徑)已接線,三條路徑全數可達。**
  ...
  對安塔蘭母星防禦艦隊開戰,戰勝即攻陷母星,下個 `EndTurn` 判定安塔蘭勝利。**母星防禦艦隊戰力
  手冊/openorion2 均無精確數字**(手冊只用「awe-inspiring」定性描述),用保守預設(6 艘末日之星
  等級戰力),已誠實標注待考證。**UI 最小化**:艦隊列表畫面加一個文字提示按鈕,沒有原版那種
  3D 場景或勝利/落敗專屬結束畫面。詳見 `docs/tech/victory-conditions.md` §4.4。)**
...
一句話(**2026-07-11 更新**):以上多為 2026-07-04~10 的描述;經 2026-07-11 的多輪接死碼+修 regression+拓殖+多 AI+安塔蘭勝利接線,**遊戲已是可玩的多帝國 4X 迴圈**(玩家與 3 AI 各自擴張、忠實+平衡經濟、可經征服/議會/安塔蘭反攻三條路徑贏一局,皆有模擬探針驗證)。先前「無法真的玩一局、只能點畫面」的描述已過時(見頭條 2026-07-11 更新)。**仍待**:戰機/武器 mod 等增強、完整 UI 打磨、以及 vs 原版還原度的使用者實測定案——這些是「讓它更完整/更像原版」,不是「能不能玩」的阻塞。
