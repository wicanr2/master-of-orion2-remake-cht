...
| 間諜／逐對手外交／條約 | `spy_test.go`、`spy_mission_test.go`、`treaty_test.go`、餽贈／RACES／外交畫面測試 | 間諜手冊 p.174-175；`SpyVsSpyAttackerBonus`；IDA `sub_53146`／`sub_533F4`／`sub_539D9`；條約與外交 raw 證據 | Docker `-gamegallery` 的 `15_diplomacy.png` 與 UI 熱區測試 | 種族關係→選 AI 列→切換 STEAL/SABOTAGE/HIDE→提議餽贈／特殊貿易／條約→終止→回合結算→存檔 | remake 操作與規則切片通過；原版評分／回應比較常數已證實，SABOTAGE 分數／特殊表與完整 AI end-to-end 接受表仍未知 |
