...
| 間諜／逐對手外交／條約 | `spy_test.go`、`spy_mission_test.go`、`original_spy_oracle_test.go`、`treaty_test.go`、`diplomacy_growth_test.go`、餽贈／RACES／外交畫面測試 | 間諜手冊 p.174-175；IDA `sub_100A3E`／`sub_100A83`／`sub_101483`、`sub_4DD6B`、`sub_4D78E` 與 `byte_180ED4` | Docker `-gamegallery` 的 `15_diplomacy.png` 與 UI 熱區測試 | 種族關係→選 AI 列→切換 STEAL/SABOTAGE/HIDE→提議餽贈／特殊貿易／條約→終止→回合關係成長／種族目標漂移→存檔 | 玩家↔AI 條約成長、14×14 目標表投影、漂移、戰爭 -90 與 SABOTAGE 帝國攻防表已閉合；AI↔AI 全矩陣、方向條約、訓練／維護、AI 任務政策、特殊貿易完整表及 end-to-end 接受表仍未閉合 |
