...
| 火線角／格子戰術 | `internal/gamedata/weapon_arc_test.go`、`internal/shell/weapon_arc_test.go`、`cmd/moo2/tacticalturn_test.go`、`tactical_layout_test.go` | `Relative_Bearing @ 0x32AD1`、`Relative_Bearing_XY @ 0x32A20`、`Move_Ship @ 0x3F5F1`、`Ship_Can_Deploy_At @ 0x49043`、手冊 p.127–128；快速 call graph 見 `docs/re/weapon-arcs.md` | 原版 640×480 公開圖 SHA-256 `77383b8a…f3354` 與 `COMBAT.LBX#0`；`-gamegallery` `16_tactical.png` | 設計→選弧→成本／佔格→建造→保存→格子戰術射擊／轉向 | 設計與格子戰術通過；640×480 戰場／控制甲板固定骨架已對齊並移除自製可見格線／卡片；原版自由座標與同狀態 DOSBox 逐像素對拍仍未完成，快速結算不消費空間射界 |
...
| 間諜／逐對手外交／條約 | `spy_test.go`、`spy_mission_test.go`、`original_spy_oracle_test.go`、`treaty_test.go`、`diplomacy_growth_test.go`、`npc_treaty_negotiations_test.go`、`ai_vs_ai_test.go` | IDA `sub_100A3E`／`sub_100A83`／`sub_101483`、`sub_4DD6B`、`sub_4D78E`、`sub_2552D`、`word_180CCC` 與鏡射鏈 | `15_diplomacy.png` 與 UI 熱區測試 | 玩家提議／終止→玩家↔AI 與 AI↔AI 關係成長→NPC ordered-pair 建約／協議／納貢→政策／議會 consumer→存檔 | 關係成長與 NPC 互不侵犯／同盟／貿易／研究／納貢分數已接；方向玩家條約、原版宣戰／停戰、`+0x5EC` producer、`+0x71F` writer、特殊貿易完整表仍未閉合 |
...
| `16_tactical.png` | 目前工作樹／Docker 畫廊建置 | 宣戰後固定 gallery 戰術 fixture；不是原版同一戰局 | 1280×960，邏輯畫布 640×480；控制甲板 y=351 | Docker + Xvfb；原版 layout 參考為不同戰局公開截圖 | 固定骨架 layout-only 通過；沒有標題、訊息帶、可見格線與逐艦卡。自由座標、縮圖與動畫仍待同狀態 oracle |
