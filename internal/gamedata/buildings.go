...
// 研究主題逐一交叉驗證)。
//
// **建造成本(PP)2026-08-07 全部換成原版真值**。先前這裡寫「手冊幾乎沒給,本檔一律是依 RP
// 研究成本量級外推的 remake 估計值」——那個缺口補掉了:原版執行檔裡有一張 49 筆、每筆 19 位元組
// 的建築表(`off_17EB3D`,由 `Real_Building_Name_` @ 0xBB40D 的 `imul eax, 13h` 指出步距),
// 欄位是
//
//	+0  dword  名稱指標(執行期才填)
//	+4  word   建築編號(自身索引)
...
//	+8  dword  **建造成本 PP**
//	+12 word   **維護費 BC/turn**
//	+14 byte   分類(7 = 軌道衛星,`Make_Bldg_Array_For_Colony_` 靠它把衛星排除在地表格點外)
//
// 怎麼確定 +8 是 PP、+12 是維護費:①Armor Barracks 的 +8 = 150,正好是先前唯一有實據的那筆
// (MANUAL_150.html modding 範例);②**+12 與手冊維護費 40/40 逐項相符**——那 40 個值是先前
// 從手冊逐條抄的,能全中代表欄位判讀與建築編號對照都對。
//
// 舊估計值錯得不算小:生態廢料場 550→200、食物複製器 460→200、歡樂穹頂 800→250、
// 星辰要塞 800→**2500**、行星屏障護盾 1200→500。
//
// 前置研究(PrereqTopic)是手冊/openorion2 tech.cpp 逐項核對過的數值,未動。
package gamedata
...
type Building struct {
	...
	// MaintenanceBC 是維護費(BC/turn)。手冊 40 項全數有給,且與原版執行檔建築表的
	// +12 欄逐項相符——那個 40/40 也正是「欄位判讀與建築編號對照都對」的佐證(見檔頭)。
	MaintenanceBC int

	// ProductionCost 是建造成本(PP),取自原版執行檔的建築表(見檔頭)。
	ProductionCost int
...
var Buildings = []Building{
	...
	},
	{
		...
		ProductionCost: 120, // 原版執行檔建築表(off_17EB3D + 8)
		PrereqTopic:    TOPIC_ADVANCED_CONSTRUCTION,
	...
	},
	{
		...
		Category: CategoryDefense, MaintenanceBC: 2,
		ProductionCost: 150, // 原版執行檔建築表(off_17EB3D + 8);亦與 MANUAL_150.html modding 範例相符
	...
	},
	{
		...
		ProductionCost: 80, // 原版執行檔建築表(off_17EB3D + 8)
		PrereqTopic:    TOPIC_ASTRO_ENGINEERING,
	...
	},
	{
		...
		Category: CategoryDefense, MaintenanceBC: 2,
		ProductionCost: 200, // 原版執行檔建築表(off_17EB3D + 8)
	...
	},
	{
		...
		ProductionCost: 100, // 原版執行檔建築表(off_17EB3D + 8)
	...
	},
	{
		...
		ProductionCost: 300, // 原版執行檔建築表(off_17EB3D + 8)
	...
	},
	{
		...
		ProductionCost: 500, // 原版執行檔建築表(off_17EB3D + 8)
	...
	// ---- 三、軌道衛星(5 項) ----
	{
		...
		ProductionCost: 400, // 原版執行檔建築表(off_17EB3D + 8)
		PrereqTopic:    TOPIC_ENGINEERING,
	...
	},
	{
		...
		ProductionCost: 2500, // 原版執行檔建築表(off_17EB3D + 8)
...
// MaintenanceBC 40 項全數手冊有給,且與原版執行檔建築表的 +12 欄逐項相符)。
//
