...
//
// 手冊有兩處寫同一件事,措辭不同但數字一致:
//   - 科技/特殊物產章:「Each unit of population assigned to science produces 5 research
//     points instead of the usual 3」(取代式)
//   - p.60 System Specials:「Relics left behind add **+2** to the amount of research
//     generated by each scientist on that planet」(相加式)
//
// 基準是 3,3+2 = 5,兩種讀法對一般種族結果相同。remake 取「5 取代基準、種族加成仍疊上去」
// 這個實作,等價於「基準 3 + 2 + 種族加成」,兩處手冊都滿足。
...
//
// **手冊獨立佐證了訂正後的結論**:p.60 System Specials 講遠古文物時寫
// 「the first empire to discover the system gets **one or two** free technology advancements」
// ——「一或兩項」,正是 `Random_(4)/4+1` 在 Random_ 回 1..n 之下的值域。
