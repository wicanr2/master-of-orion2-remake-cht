...
// starting_random_tech.go 保留早期「主題級」近似 helper 與回歸測試。
// 現行先進級開局不再呼叫這條路徑；實際玩家路徑在
// starting_original_application.go 依 `sub_FD335` 直接評分科技應用。
//
// ============ 這個缺口從第 30 項(TECH LEVEL 第二效果)就開著 ============
//
...
//
// 這些 helper 以 weight=1 封存舊行為，不得再當成現行原版 parity 路徑。
...
//
// ⚠ 此歷史 helper 的基礎 weight 固定為 1；現行玩家路徑不使用它。
func StartingRandomTopicScores(available []ResearchTopic, researchPerTurn int) []int {
	...
	for horizon := StartingRandomHorizonInitial; ; horizon = startingRandomHorizonGrow(horizon) {
		...
		for i, t := range available {
			...
			scores[i] = horizon / turns // 歷史近似 helper 固定 weight=1
			if scores[i] > 0 {
				any = true
			}
		}
...
// ============ 粒度:原版挑的是「科技應用」,不是「主題」 ============
//
// 2026-08-08(第 49 項(安塔蘭防禦艦隊))發現的一個比「weight = 1」嚴重得多的落差。
//
