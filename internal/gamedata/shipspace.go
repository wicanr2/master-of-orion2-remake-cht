...
var WeaponSpaceByName = map[string]int{
	...
	// 陀螺去穩器(第 139 項,手冊特殊武器表的 Size 欄)。
	//
	// ⚠ 那張表在 PDF 抽取後欄位是打散的:五個武器名/效果先出來,接著才是 Size 欄的
	// 五個數字 `20 75 30 50 40`(反飛彈火箭 / 陀螺去穩器 / 牽引光束 / 脈衝星 / 電漿網)。
	// **對齊靠的是脈衝星 = 50 正好對上已經從 p.127 記下來的值**——第四項對上了,
	// 整列的對應就確定了,不必猜。
	//
	// 同一列還讀到:反飛彈火箭 20、牽引光束 30、電漿網 40、停滯力場 75。
	// 那四個目前走 SpecialOptions(建造成本)而不是這張佔格表,先記在這裡;
	// 等 remake 有逐元件佔格的造艦模型時直接用。
	"陀螺去穩器": 75, // Gyro Destabilizer
}
