...
//
// ============ 為什麼它們拖到第 69 項(戰鬥速度與引擎階)才做 ============
//
// 第 64 項(武器傷害真表)盤點手冊 p.127 的特殊武器時,把這兩個判定成「卡在機制」:
//
//	牽引光束    reduces target speed  → 戰鬥速度模型
//	停滯力場    suspended animation   → 「這一輪不能動」的狀態
//
// 第 69 項(戰鬥速度與引擎階)建了戰鬥速度、第 69 項(戰鬥速度與引擎階)建了移動預算。**缺的最後一塊是「讓對方動不了」**
...
// 這與第 64 項(武器傷害真表)球形武器的「級數 = index+1」是同一個讀法,兩處互相支持。
//
// 停滯力場:
//
