...
//     WeaponOptions 已掛載 Pulsar／Spatial Compressor／Gyro Destabilizer；Plasma Flux
//     由事件怪物藍圖進入同一分類，Engine Explosion 另屬爆炸事件；死光仍是一般光束武器。
//
...
func weaponKindByName(name string) WeaponKind {
	switch name {
	case "脈衝星", plasmaFluxName, spatialCompressorName, gyroDestabilizerName:
...
// WeaponKindForName 提供顯示／戰術 adapter 使用與核心相同的武器分類，避免 UI 再維護一份
// 會漂移的名稱 switch。
func WeaponKindForName(name string) WeaponKind { return weaponKindByName(name) }
...
func effectiveArmorHP(sh Ship) int {
	...
	if shipHasSpecial(sh, heavyArmorName) {
