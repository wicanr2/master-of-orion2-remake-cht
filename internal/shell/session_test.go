...
func TestGameSessionEndTurn(t *testing.T) {
	...
	if s.LastPlayerOutput.TotalResearch != 30 {
		t.Errorf("玩家總研究 = %d,預期 30", s.LastPlayerOutput.TotalResearch)
	}
	if s.Player.ResearchProgress != 30 {
		t.Errorf("玩家研究進度 = %d,預期 30", s.Player.ResearchProgress)
	}
	// AI 也推進了(研究進度增加)
