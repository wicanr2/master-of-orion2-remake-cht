...
func TestHotseatCanSelectSpecificAIEmpires(t *testing.T) {
	...
	s.AIReputationRaw = [][]int{{0, 10, 20}, {30, 0, 40}, {50, 60, 0}}
	...
	s.AIIncidentReasonRaw = [][]int{{0, 4, 5}, {14, 0, 4}, {5, 14, 0}}
	s.AIIncidentMagnitudeRaw = [][]int{{0, -8, 6}, {4, 0, -5}, {7, 3, 0}}
	s.AIIncidentMemoryRaw = [][]int{{0, 1, 2}, {1, 0, 3}, {2, 3, 0}}
	s.AIIncidentBetrayalRaw = [][]bool{{false, true, false}, {false, false, true}, {true, false, false}}
	...
	if len(s.AIReputationRaw) != 1 || s.AIReputationRaw[0][0] != 0 ||
		...
		len(s.AIIncidentReasonRaw) != 1 || len(s.AIIncidentMagnitudeRaw) != 1 || len(s.AIIncidentMemoryRaw) != 1 ||
		len(s.AIIncidentBetrayalRaw) != 1 ||
