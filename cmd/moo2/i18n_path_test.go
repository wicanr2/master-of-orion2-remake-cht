...
// 換成烘進執行檔的版本之後,**從別的目錄跑仍然失敗**——還有七處直接 `os.Open` 單一 .tsv,
...
func TestNoHardcodedI18NPath(t *testing.T) {
	...
	for _, e := range entries {
		...
		if e.Name() == "embedassets.go" || strings.HasSuffix(e.Name(), "_test.go") {
		...
		// (`interactive.go` 的第二個譯表載入點,字面已經改成 "menu.tsv" 卻還走 os.Open,
	...
	if len(bad) > 0 {
		t.Errorf("這些檔案寫死了 assets/i18n 路徑,改用 OpenI18NTSV / i18nFS:%v", bad)
	}
}
