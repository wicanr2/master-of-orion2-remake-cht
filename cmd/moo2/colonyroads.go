...
// ============ 五、⚠ 還不能宣稱與原版逐格相同 ============
//
// 亂數流的**結構**已經接完整了:起種 → 逐棟插入 → 排序 → 8 輪房屋抖動 → 找國會大廈
// (只掃描,交換沒模擬)→ 道路。每一步消耗幾次亂數都照抄,護欄測試逐項釘住。
//
// 剩下的差異在**建築集合**,不在演算法:原版走的是殖民地的 49 個「有沒有這棟」旗標,
// 裡面包含 remake 沒有建模的項目(Colony Base、已完成的一次性改造等)。
// 集合差一棟,蓄水池抽樣從第一步就錯開,後面全部跟著偏。
//
// 所以:**美術、格點、方向規則、密度、流程都是原版真值;「同一顆星會不會長一模一樣」
// 還沒對原版實測過,不要當成已驗證。**
