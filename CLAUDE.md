...
> (196 個位址、131 列表格——全 repo 最貴的內容)+ 150 個編號項。
> **開頭那張「這 150 項煉出來的規則」12 條是唯一該讀完的部分**,底下 150 項是參考資料。
>
...
>   **任何百分比都是寫下當日的快照**,而且本專案不自評這個數字。
> - 「最高優先剩 ③按鍵像素對齊」是 2026-07-12 的快照。**2026-08-07 已作廢**:
>   像素對齊的待重挖名單已清空(反組譯挖立即數那條路走完),剩下的不是座標沒挖而是子系統沒做。
> - ⚠ **最危險的一個:「已經沒有清楚定義的自驅剩項了」。**
>   這句話 2026-07-12 寫進過文件與長期記憶,而之後 gap-report 第 52–72 項光是手冊忠實化
>   就找出數十項。**「沒有剩下的了」會讓下一輪不去找**,所以它比任何一個具體的錯誤都貴。
>   2026-08-08 已從記憶與 `docs/REMAKE-COMPLETION-ASSESSMENT.md` 刪除。
>
...
> 逐位元比對過場截圖廊(34 張)是本專案實際在用的驗收測試;`30_netwait.png` 上的**狀態指紋**
> 一旦改變,就代表持久化狀態變了,**必須查出原因**,不可因為「改動有理」就放過。
>
> **盤點紀律(2026-08-08 第 72 項(元件表有≠效果有接)學到的,會重複踩)**:
> - **「元件表裡有這一項」不等於「效果有接」。** 隱形裝置在元件表裡躺了很久,而程式碼沒有任何
>   地方讀它——先前每一次盤點問的都是前者,所以每次都被算成已完成。要分開問這兩件事。
> - **擋門理由會過期,而且沒有任何機制會提醒你**(第 52–72 項撞了八次)。更糟的一種是
>   **當初就寫錯**:「手冊沒有公開這個數字」往往只表示沒查過。標「近似」的前提是**真的查過**。
> - **任何清單都會過期,包括警告別人清單會過期的那份清單。** `ship_systems.go` 檔尾那份
>   「沒接的與理由」12 項裡有 7 項早就接掉了。做完一項就回去刪那一行。
