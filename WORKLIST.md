...
- [x] 議會畫面(COUNCIL.LBX#1)接入(種族關係→會晤→銀河議會)+ 標題中文化
...
- [ ] colony 名稱改用真星名;建造欄填建造佇列;4 列表畫面接真存檔/生成資料
- [x] 星圖互動:點星→黃色高亮環+左下角行星資訊面板(名/氣候/大小/重力/礦產)
- [ ] 星圖接真實星系生成(galaxy.go -save 移植);外交對談 DIPLOMAT#29 調色盤雜訊未解
...
- [ ] 艦隊移動 + 星圖導航
- [ ] 艦艇設計
- [~] 戰鬥:shell 最小戰鬥解算(艦隊戰力比較+損失)接種族關係「宣戰」→戰鬥結果畫面;完整戰術戰鬥待做
- [ ] 外交
- [ ] 隨機事件 / 安塔蘭人
