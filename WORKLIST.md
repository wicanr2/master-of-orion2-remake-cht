...
| B | **`sub_FC845` 逐函式抄寫**(985 行,AI 科技估值 `weight`):第 88/91/99 項三度判定「一次讀完風險太高」,目前 `weight` 一律當 1 | RE,方法論齊備、錨點明確 | sonnet |
| C | **文件過期斷言掃描**:docs/ 橫跨數十輪,方框與敘述大量過期;抽可驗證斷言回 code grep | 機械核實 | sonnet |
| D | **spy / leader / 多 AI 目標選擇 UI**:目前是文字提示疊原版底圖 | 需先挖座標,再實作 | sonnet 挖座標 → 主迴圈定案 |
| E | **AI 艦隊移動模型**:AI 沒有星圖移動,連帶阿特米斯地雷打不到 AI、AI 艦艇沒有艦員經驗 | 架構級 | 主迴圈 |
| F | **戰機基地 10 回合整補**:remake 沒有駐軍耗損機制,單獨補計時器等於空轉 | 需先建耗損 | 待 E 之後 |
| G | **屏障護盾擋生物武器 / 護盾被炸後氣候回退**:前者無「生物武器」分類,後者是刻意不做(建築效果一律不可逆) | 已記錄的留白 | 不做 |
| H | **安塔蘭母星艦隊強度**:無一手數字 | 待 RE | 低優先 |
| I | **turn-1 數值 playtest 校準** | 實測 | sonnet |
