...
| 項目 | 現況 |
|---|---|
| **英文模式的引擎層** | ✅ **1,501 條的名稱池已接回**(2026-08-08 第 84 項(名稱池雙語化)):池子改存英文原文,中文由注入的翻譯器在取名當下翻,中文模式輸出逐位元不變。**剩下的**:開局艦隊硬編名(`拓荒號`/`先驅一號`)、艦級名(`殖民船`/`偵察艦`)、以及引擎產生的敘述字串。⚠ `internal/` 的 2,471 條裡有一大批是**查表 key** 不是顯示字串(`special_device_map` 31、`weapon_damage` 25、`shipspace` 25…),下一輪要先分類再動手 |
...
| **2× 邏輯畫布(1280×960)** | `rulebook/81` 的處方,但 remake 已經是 640×480(原版美術原生解析),所以不是「低解析太小」而是「CJK 想要更大字」。成本量過:**420 個繪製呼叫點**(50 `DrawImage` / 158 `vector.*` / 198 文字)+ 命中區映射。**獨立一輪** |
...
| BL | **英文版面撞牆,一層層挖**(2026-08-08 第 85 項(元件名英文)):艦艇設計畫面六層破版——右緣截字 → 壓分隔線 → 豆腐框 `▸` → 元件名沒英文 → 改造標籤沒英文 → 補完英文之後標籤疊在一起。**四層是英文專屬,中文截圖一張都看不到**。順手把改造晶片的熱區與繪製收成同一個座標函式(先前兩份寫死表)。⚠ **2× 畫布沒做**:量過是 420 個繪製呼叫點 + 命中區映射,獨立一輪 | UI | 主迴圈 ✅ |
