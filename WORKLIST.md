...
| ~~把反組譯出來的音樂對照表接進 remake~~ **已完成**(2026-08-08 第 78 項(音樂接線)) | 單一編號空間、兩個 LBX 都載、主選單/星圖每次重擲。**剩兩個講明的缺**:科學室播完不接下一首(原版的 `-2` 哨兵,remake 的 Mixer 沒有這個機制);外交只有「關係差」那一組(「關係好」是逐族資料表,還沒追出來) |
