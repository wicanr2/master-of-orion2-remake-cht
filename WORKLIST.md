...
| 元件 | 手冊規則 | 為什麼現在做得了 |
|---|---|---|
| **隱形裝置** | 未攻擊時 +80 光束防禦、飛彈與魚雷 50% 未命中;開火即失效;整整一回合沒開火才能重新隱形 | ⚠ **已經在 `SpecialOptions` 裡、花得了錢、整份程式碼沒有任何地方讀它**。最後那句的語意就是第 70 項(陀螺去穩器)建好的 `Fired` |
| 能量吸收器 | 1/4 傷害轉存,可回射且自動命中(除非目標有位移裝置),傷害隨距離衰減 | 第 70 項(陀螺去穩器)已建跨回合的 `Charged`/`Fired`;位移裝置第 68 項(元件盤點+飛彈防禦)也在 |
...
| ④ 手冊有而沒抄的資料 | 見上面第一節 |
| **新增:元件表有但沒有消費端** | 第 72 項(元件表有≠效果有接)起才會問這一題。目前 1 項(隱形裝置)+ 1 項已標註的代理(重生程序) |
...
| **把反組譯出來的音樂對照表接進 remake** | 場景→曲目已從執行檔全部解出(第 73 項(音樂場景表))。⚠ remake 現行把主選單當固定曲目,而**原版是 `clock()%3+1` 每次隨機三選一**——這是行為差異,要改 |
...
| Q | **艦員經驗只加攻擊沒加防禦**(2026-08-08 第 60 項):註解寫「打得準也閃得掉」而程式只做前半;飛彈閃避恆傳 0 的六個來源裡有兩個現在算得出來(艦員/舵手)。登艦戰那一支仍不接——那個系統確實不存在,理由沒過期 | 覆蓋率探針 + 手冊 | 主迴圈 ✅ |
...
| AW | **150 項煉成 12 條規則**(2026-08-08 第 74 項):使用者「150 條太多了」。那 150 個編號項裡的教訓**高度重複**——「擋門理由會過期」出現十次、「兩條戰鬥路徑」出現在第 68–71 項每一項。合併成 12 條放在 gap-report 最前面(判斷還缺什麼 5 條 / 改動完整性 4 條 / 逆向 3 條 + 驗收),每條標出產生它的項次。**150 項降級成參考資料**,要看某條規則的證據再去翻。`CLAUDE.md` / `CONTEXT.md` 的指標同步改成「只有那 12 條該讀完」 | 文件 | 主迴圈 ✅ |
...
| BA | **複查抓到兩個真的改壞**(2026-08-08 第 76 項):①**跨文件的項次被套用了 gap-report 的對照表**——`rules-implementation-audit.md 第 10 項` 變成 `第 3 項`、`doc-audit 第 3/4 項` 變成 `第 2 項`,五處已還原。**被改成一個存在的錯項次,「有沒有失效引用」那個檢查抓不到**。②短標貼到同編號的另一個小節上(舊 `第 60 項(分艦隊)` → `第 23 項(分艦隊)`,而第 23 項正名是多艦隊模型),含 WORKLIST 49 列日誌。現在全 repo 同一項次只有一個短標 | 文件 | 主迴圈 ✅ |
...
- [x] 第 47 項(AI艦隊移動)+ 第 55 項(AI自己研究)之後,三塊都已完成
...
- [x] **熱座(hotseat)**(2026-08-07):多位真人同機輪流下令。席位交換模型
  ...
  席位跑——兩點都寫在 `advanceIdleSeats` 檔頭與 `docs/re/01-gap-report.md` 第 3 項(殖民地/事件畫面)。
...
第 14 項(地表道路)補母星的國會大廈時只治了一半。**編號 11 Colony Base 是完全對稱的另一半**:
...
## ★ 2026-08-07 戰術格子的獨立戰機單位 + 一個讀錯的欄位(gap report 第 27 項(戰機中隊單位))
...
原版 y=430 那列是聊天輸入(**2026-08-07 已補上**,見第 36 項(聊天列資料))。
...
(**2026-08-07 補上了**,見下面第 36 項(聊天列資料)。)

## ★ 2026-08-07 TECH LEVEL 的第二個真效果(gap report 第 30 項(科技等級第二效果))
...
拿這份清單 × 第 30 項(科技等級第二效果)的六個開局主題 × remake 的建築前置表跑一遍,科技條件成立的
...
因為第 30 項(科技等級第二效果)留的 19 個隨機主題還沒發。測試附正對照:科技全解時這套機制確實會發滿 6 個。
...
## ★ 2026-08-07 聊天列(gap report 第 36 項(聊天列資料))
...
**又撞到同一組六個主題**:`sub_FD2F9` 的六個硬編位址減 `0xC4` 正好是第 30 項(科技等級第二效果)的六個開局主題
...
已經有了而且都對得上手冊**(BA/BD 在 `formulas.go`、ME 在 `missile.go`),而且**接到船上了**——`Ship.CrewXP` 欄位在,等級由 `shipCrewLevel` 現算,新造艦依殖民地起始經驗;第 60 項(艦員防禦)把它接上攻擊與防禦兩側。
...
這個缺口從第 30 項(科技等級第二效果)就開著,第 88、91 兩次判斷「不照抄 `sub_FC845` 那 985 行」。
這次換個問法:**擋住的到底是哪一部分?**
...
`[player+0x28]` 的二次過濾**不照抄**(那個欄位沒查到寫入端,第 36 項(聊天列資料)也遇過同一個)。
