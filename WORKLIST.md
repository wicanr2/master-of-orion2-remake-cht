...
> **這一段是唯一該看的剩餘工作表。** 底下各 Phase 的方框橫跨數十輪,已不可靠——
> 那 32 個 `[ ]` 裡多數在後續輪次做掉了(例如「OCR 掃描版手冊附錄」在第 123 項用可抽文字的
> patch 1.5 PDF 解決、「決定性化 + lockstep」在第 72–79 項整塊完成),**保留只為了時序,
> 不是現況**。要判斷某項做了沒,`grep` 程式碼,不要讀方框。
>
