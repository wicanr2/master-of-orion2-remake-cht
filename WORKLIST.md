...
- **原版 PRNG + 原版擺放演算法 + 地表底圖**(gap report 第 31 項,同日):
  ...
  - 仍缺:擺放的最後一段隨機微調沒做(**道路也吃這條亂數流**,見第 41 項)、植被層沒做、
...
⚠ **道路位置不會與原版同一局逐格相同**:它吃的是房屋抖動之後的亂數流位置,而抖動還沒補。
美術、格點、方向規則、密度是原版真值,走法不是。

**新發現的缺口**:道路之後原版還跑一層**植被**(`COLVEGGI.LBX`,每個空格子最多 2 株,
密度看周圍道路數),remake 整層沒有。
