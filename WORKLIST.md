...
| B | **`Calc_Tech_Value_` 抄寫** — 規格 + 三張資料表已解(`docs/re/calc-tech-value.md`、`calc-tech-value-tables.md`),**低風險那半已接**(category 倍率 + 科技應用粒度);高風險的階段 C–K 仍擱置。⚠ **擋門換了**:category enum 語意 2026-08-08 第 111 項已從成員反推出來(41 個乾淨的功能類別),現在卡的是 `player[0x205]`/`[0x206]`/`[0x28]`/`[0x89F..0x8BB]` 這幾個欄位——**沒查到寫入端**,語意純屬形狀推測 | RE | sonnet ✅ + 主迴圈接線 |
