...
>
> **本週收尾快照（2026-08-27）**：依本段唯一活表重新核算，38 項中 26 項完成、
> 6 項進行中、6 項待辦；完成計 1、進行中計 0.5，remake 功能完成度維持 **76%**。
...
- [~] **全畫面動態文字安全框（2026-08-12，進行中）**：檢測與回歸流程已寫入
  ...
  2026-08-27 重新逐張檢視 README 實際引用圖後，確認 `docs/screenshots/25_shipdesign.png` 的
  「各艦體空間」標題／六列跨過原版上下內容面板的分隔框；既有測試只驗文字安全框互不重疊，
  沒有驗證安全框是否包含於背景美術的 panel rectangle，因此先前畫廊綠燈不能關閉此缺口。
  `docs/screenshots/23_multiplayer.png` 的熱座說明雖在 640×480 畫布內，卻位於 482×335 主面板外，
  視覺上形成脫框；要移回面板或保留外置說明須先完成版面取捨。README 在兩張遊戲實圖修正並
  重跑繁中／英文畫廊前不得替換成新的「已驗收」截圖。後續幾何測試必須同時驗證文字安全框
  `contained by` 所屬美術面板，而不只驗文字框彼此 `non-overlap`。
