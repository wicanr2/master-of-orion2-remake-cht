...
> **目前定位：可玩 remake 預覽，不是已完成的原版忠實重製。** 截至 2026-08-27 的本週收尾盤點，
> 2026-08-25 啟動的 IDA Pro
> 重新稽核確認，部分議會、轟炸、外交、AI、經濟與戰鬥規則仍採 remake 自行設計或近似模型。
> 專案現已把原版玩家機制重新列為最高優先，逐項建立「原版函式與資料流 → Go 規則 → UI／存檔／
> 測試」的證據鏈。稽核結果見
> [原版忠實度重新稽核](docs/re/parity-re-audit-20260812.md)；逐系統進度見
> [原版忠實度矩陣](docs/re/parity-matrix.tsv)。

### Remake 自評儀表板（2026-08-27 本週收尾）

| 維度 | 自評 | 可重算口徑 |
|---|---:|---|
| remake 功能完成度 | **76%** | `WORKLIST.md` 唯一活表 38 項：26 項完成、6 項進行中、6 項待辦；完成計 1、進行中計 0.5，故 `(26 + 6×0.5) / 38`，四捨五入為 76% |
| 原版玩法對齊度 | **49%** | `docs/re/parity-matrix.tsv` 40 個玩家系統依「原版對齊狀態」欄分級：10 項以「已對齊」收束、19 項為其餘部分證據、11 項以「未知／模型不同／不是原版 AI」收束；部分證據計 0.5，故 `(10 + 19×0.5) / 40` |
| 發行驗證完成度 | **33%** | 三個獨立閘門中，打包工程抽樣已通過；最新工作樹重打包／三平台真機與外部音訊逐曲人耳驗收尚未完成，故 `1 / 3` |

這三個百分比是當日工程快照，不是單一總分。尤其「原版玩法對齊度」不代表逐幀、逐像素、
逐位元或原版亂數序列一致；單元測試通過也不會自動增加這一項。本週完成的文案外部化切片
仍歸在同一個進行中母項，原版矩陣分級與發行閘門也未跨級，因此三項分數與前次快照相同。
...
- **兩種戰鬥體驗**：快速戰鬥結算與格子戰術戰鬥分開處理，包含光束、飛彈、戰機、地面戰、
  安塔瑞斯終局與戰鬥視覺特效；部分戰機、轟炸、防禦與地面戰資料流仍在原版忠實度稽核中。
...
遊戲使用 640×480 邏輯畫布；輸出可以整數倍放大，保留原版像素比例。以下為目前版控中的
以目前工作樹錄製與 `-gamegallery` 產出的代表畫面；新局、種族與命名畫格已抽樣檢查文字均落在
可見按鈕／對話框內，並非原版逐像素對照：
